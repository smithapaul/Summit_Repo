DROP VIEW CSSTG_OWNER.BB_ACTIVITY_TERM_UMLOW_DAY_VW
/

--
-- BB_ACTIVITY_TERM_UMLOW_DAY_VW  (View) 
--
CREATE OR REPLACE VIEW CSSTG_OWNER.BB_ACTIVITY_TERM_UMLOW_DAY_VW
BEQUEATH DEFINER
AS 
with Q1 as (
select /*+ parallel(8) inline no_use_nl(ACT CM U) */
       'UMLOW_DAY' AS INSTITUTION_CD,
       case when CM.BATCH_UID like 'L%' and substr(CM.BATCH_UID,1,4) <> 'LOCE' and substr(CM.BATCH_UID,2,1) between '0' and '9'
            then substr(CM.BATCH_UID,2,4)
            when CM.BATCH_UID like 'L%' and substr(CM.BATCH_UID,1,4) = 'LOCE' and substr(CM.BATCH_UID,5,1) between '0' and '9'
            then substr(CM.BATCH_UID,5,4)
            else NULL
        end as TERM_CD,
       case when CM.BATCH_UID like 'L%' and substr(CM.BATCH_UID,1,4) <> 'LOCE' and substr(CM.BATCH_UID,7,1) between '0' and '9'
            then substr(CM.BATCH_UID,7,5)
            when CM.BATCH_UID like 'L%' and substr(CM.BATCH_UID,1,4) = 'LOCE' and substr(CM.BATCH_UID,10,1) between '0' and '9'
            then substr(CM.BATCH_UID,10,5)
            else NULL
        end as CLASS_NUM,
