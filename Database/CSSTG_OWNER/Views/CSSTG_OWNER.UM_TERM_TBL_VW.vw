DROP VIEW CSSTG_OWNER.UM_TERM_TBL_VW
/

--
-- UM_TERM_TBL_VW  (View) 
--
CREATE OR REPLACE VIEW CSSTG_OWNER.UM_TERM_TBL_VW
BEQUEATH DEFINER
AS 
WITH
        T1
        AS
            (SELECT INSTITUTION,
                    ...
                    NVL (
                        ...
                        TO_DATE ('31-DEC-9999'))    NEXT_BEGIN_DT,
                    NVL (
                        MAX (TRUNC (TERM_BEGIN_DT))
                            OVER (
                                PARTITION BY INSTITUTION,
                                             ACAD_CAREER,
                                             SRC_SYS_ID
                                ...
                                ROWS BETWEEN UNBOUNDED PRECEDING
                                     AND     1 PRECEDING),
                        TO_DATE ('01-JAN-1900'))    PREV_BEGIN_DT,
                    NVL (
                        MAX (
                            CASE
                            ...
                            OVER (
                                ...
                                ROWS BETWEEN UNBOUNDED PRECEDING
                                     AND     1 PRECEDING),
                        TO_DATE ('01-JAN-1900'))    PREV_FULL_TERM_BEGIN_DT
            ...
            (SELECT INSTITUTION,
                    ...
                    LEAST (
                        NVL (
                            ...
                            TO_DATE ('01-JAN-1900')),
                        TERM_BEGIN_DT)              EARLY_BEGIN_DT,
                    NVL (
                        MIN (TRUNC (TERM_BEGIN_DT) - 1)
                            OVER (
                                PARTITION BY INSTITUTION,
                                             ACAD_CAREER,
                                             SRC_SYS_ID
                                ...
                                ROWS BETWEEN 1 FOLLOWING
                                     AND     UNBOUNDED FOLLOWING),
                        TO_DATE ('31-DEC-9999'))    LATE_END_DT,
                    PREV_BEGIN_DT,
                    PREV_FULL_TERM_BEGIN_DT
               FROM T2),
        T4
             ...
             SELECT INSTITUTION,
               ...
               FROM (SELECT INSTITUTION,
                            ...
                            NVL (
                                MAX (
                                    CASE
                                        WHEN (   SUBSTR (STRM, -1, 1) IN
                                                     ('2', '6')
                                              OR SUBSTR (STRM, -2, 2) IN
                                                     ('10', '30'))
                                        THEN
                                            TRUNC (TERM_BEGIN_DT)
                                        ELSE
        ...
        AS
            (SELECT INSTITUTION,
                    ...
                    (CASE
                         WHEN     STRM >= '1010'
                              AND SUBSTR (STRM, 3, 2) IN ('10', '30')
                         THEN
                             'Y'
                         WHEN     STRM < '1010'
                              AND SUBSTR (STRM, -1, 1) IN ('2', '6')
                         THEN
                             'Y'
                         ELSE
                             'N'
                    ...
                    MIN (TERM_BEGIN_DT)
                        OVER (
                            ...
                            ORDER BY STRM)    MIN_TERM_BEGIN_DT
               FROM T4)
    SELECT INSTITUTION,
           ACAD_CAREER,
           STRM,
           ...
           TERM_END_DT,
           CASE
               WHEN TERM_BEGIN_DT = MIN_TERM_BEGIN_DT
               THEN
                   TO_DATE ('01-JAN-1900')
               ELSE
                   EARLY_BEGIN_DT
           ...
           NVL (
               CASE
                   WHEN T5.DESCR LIKE '%Summer' AND T5.INSTITUTION = 'UMLOW'
                   THEN
                       LEAD (T5.STRM, 2, NULL)
                           OVER (
                               PARTITION BY T5.INSTITUTION,
                                            T5.ACAD_CAREER,
                                            T5.SRC_SYS_ID
                               ORDER BY T5.STRM)
                   ELSE
