DROP TABLE CSSTG_OWNER.UMOL_ACTIVITY_ACCUMULATOR_UMLOW_DAY CASCADE CONSTRAINTS
/

--
-- UMOL_ACTIVITY_ACCUMULATOR_UMLOW_DAY  (Table) 
--
CREATE TABLE CSSTG_OWNER.UMOL_ACTIVITY_ACCUMULATOR_UMLOW_DAY
(
...
COMMENT ON COLUMN CSSTG_OWNER.UMOL_ACTIVITY_ACCUMULATOR_UMLOW_DAY.DATA IS 'This contains any free-form text data associated with the event. Data related to the event. This field provides additional information on the event. Each event uses this field, if at all, in different ways. In many instances, the data included in this field is a value that is easily deduced. In some instances, the value in the data field is a number, such as _1_1 or _29_1. These are associated with an EVENT_TYPE of TAB_ACCESS or MODULE_ACCESS. The first number is an ID that references a tab or a module (the second number can be ignored). The ID number that refers to a tab can be found in the user interface by scrolling over the tab. The ID number appears as part of the URL in the status bar.The ID number that refers to a module can be found in the same manner. Scroll over the Maximize, Minimize, or Edit button associated with a module. The ID number will appear as part of the URL in the status bar. Please note that the Detach Module button returns a different ID number and should be ignored.'
/

COMMENT ON COLUMN CSSTG_OWNER.UMOL_ACTIVITY_ACCUMULATOR_UMLOW_DAY.TIMESTAMP IS 'This is the date and/or time at which this event occurred.'
/

COMMENT ON COLUMN CSSTG_OWNER.UMOL_ACTIVITY_ACCUMULATOR_UMLOW_DAY.STATUS IS 'This indicates the status of the event. Value values are success (1) or failure (0). The values come from the TrackingEvent.Status enumeration.'
/

COMMENT ON COLUMN CSSTG_OWNER.UMOL_ACTIVITY_ACCUMULATOR_UMLOW_DAY.SESSION_ID IS 'This is a reference to the primary key of the user session associated with this activity. Identifies the user session that initiated the action. A session is simply a browser connection to the system launched from an end-user machine.'
/

COMMENT ON COLUMN CSSTG_OWNER.UMOL_ACTIVITY_ACCUMULATOR_UMLOW_DAY.DELETE_FLAG IS 'This is a Summit Y/N flag that indicates that this row is no longer in the source data.'
/

COMMENT ON COLUMN CSSTG_OWNER.UMOL_ACTIVITY_ACCUMULATOR_UMLOW_DAY.INSERT_TIME IS 'This is the date and time that this row was inserted into the Summit database.'
/

COMMENT ON COLUMN CSSTG_OWNER.UMOL_ACTIVITY_ACCUMULATOR_UMLOW_DAY.UPDATE_TIME IS 'This is the date and time that this row was last updated in the Summit database.'
/
