DROP PROCEDURE CSMRT_OWNER.PS_LOAN_ORIGNATN_P
/

--
-- PS_LOAN_ORIGNATN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_LOAN_ORIGNATN_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_LOAN_ORIGNATN';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_LOAN_ORIGNATN@SASOURCE S)
 where TABLE_NAME = 'PS_LOAN_ORIGNATN'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_LOAN_ORIGNATN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_LOAN_ORIGNATN';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_LOAN_ORIGNATN T
using (select /*+ full(S) */
...
nvl(trim(SFA_DL_CRD_DEC_ST),'-') SFA_DL_CRD_DEC_ST
  from SYSADM.PS_LOAN_ORIGNATN@SASOURCE S
 where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOAN_ORIGNATN')
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_LOAN_ORIGNATN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_LOAN_ORIGNATN';
update CSSTG_OWNER.PS_LOAN_ORIGNATN T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, ACAD_CAREER, LOAN_TYPE, LN_APPL_SEQ
   from CSSTG_OWNER.PS_LOAN_ORIGNATN T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOAN_ORIGNATN') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, ACAD_CAREER, LOAN_TYPE, LN_APPL_SEQ
   from SYSADM.PS_LOAN_ORIGNATN@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOAN_ORIGNATN') = 'Y'
   ) S
