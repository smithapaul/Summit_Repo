DROP PROCEDURE CSMRT_OWNER.UM_F_ADM_APPL_LST_SCHL_P
/

--
-- UM_F_ADM_APPL_LST_SCHL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."UM_F_ADM_APPL_LST_SCHL_P" AUTHID CURRENT_USER IS
...
insert /*+ append enable_parallel_dml parallel(8) */ into UM_F_ADM_APPL_LST_SCHL
with
ADM as (
...
--                              order by CONVERTED_GPA desc, EXT_SUMM_TYPE_ID desc, EXT_ACAD_CAR_ID desc, EXT_DATA_NBR) EXT_ORDER   -- Aug 2019
       row_number() over (partition by PERSON_SID, INSTITUTION_SID, EXT_ORG_SID, SRC_SYS_ID
                              order by EXT_SUMM_TYPE_ID desc, EXT_ACAD_CAR_ID desc, EXT_DATA_NBR desc) EXT_ORDER   -- July 2022
  from PS_F_EXT_ACAD_SUMM
 where DATA_ORIGIN <> 'D'
   and EXT_SUMM_TYPE_ID <> 'PBOV'
   )
select ADM.APPLCNT_SID,
       ...
       decode(EXT.UM_CUM_CREDIT_AGG,0,NULL,EXT.UM_CUM_CREDIT_AGG) UM_CUM_CREDIT_AGG,    -- Aug 2022
       decode(EXT.UM_CUM_GPA_AGG,0,NULL,EXT.UM_CUM_GPA_AGG) UM_CUM_GPA_AGG,             -- Aug 2022
       decode(EXT.UM_CUM_QP_AGG,0,NULL,EXT.UM_CUM_QP_AGG) UM_CUM_QP_AGG,                -- Aug 2022
