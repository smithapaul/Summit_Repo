DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_PKG_VAR_P
/

--
-- AM_PS_STDNT_PKG_VAR_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_PKG_VAR_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STDNT_PKG_VAR';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STDNT_PKG_VAR@AMSOURCE S)
 where TABLE_NAME = 'PS_STDNT_PKG_VAR'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_STDNT_PKG_VAR';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_STDNT_PKG_VAR';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_PKG_VAR T
using (select /*+ full(S) */
  ...
  from SYSADM.PS_STDNT_PKG_VAR@AMSOURCE S 
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_PKG_VAR')
   and EMPLID between '00000000' and '99999999'
   and length(EMPLID) = 8
   and EMPLID <> '22786786' ) S     -- Jan 2022 bad data. 
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_PKG_VAR';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_PKG_VAR';
update AMSTG_OWNER.PS_STDNT_PKG_VAR T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, ACAD_CAREER
   from AMSTG_OWNER.PS_STDNT_PKG_VAR T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_PKG_VAR') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, ACAD_CAREER
   from SYSADM.PS_STDNT_PKG_VAR@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_PKG_VAR') = 'Y'
   ) S
