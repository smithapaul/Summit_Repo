...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_BI_BILLING_LINE';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_BI_BILLING_LINE@SASOURCE S)
 where TABLE_NAME = 'PS_BI_BILLING_LINE'
;
...
strSqlCommand   := 'merge into CSSTG_OWNER.PS_BI_BILLING_LINE';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_BI_BILLING_LINE T
using (select /*+ full(S) */
...
from SYSADM.PS_BI_BILLING_LINE@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_BI_BILLING_LINE')
...
strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_BI_BILLING_LINE';
update CSSTG_OWNER.PS_BI_BILLING_LINE T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
