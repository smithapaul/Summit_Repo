DROP PROCEDURE CSMRT_OWNER.AM_PS_INSTRUCT_MODE_P
/

--
-- AM_PS_INSTRUCT_MODE_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_INSTRUCT_MODE_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_INSTRUCT_MODE@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_INSTRUCT_MODE';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_INSTRUCT_MODE';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_INSTRUCT_MODE';

   MERGE /*+ use_hash(S,T) */
        INTO  AMSTG_OWNER.PS_INSTRUCT_MODE T
        USING (SELECT /*+ full(S) */
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_INSTRUCT_MODE';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_INSTRUCT_MODE';

   UPDATE AMSTG_OWNER.PS_INSTRUCT_MODE T
      SET T.DATA_ORIGIN = 'D', T.LASTUPD_EW_DTTM = SYSDATE
    WHERE     T.DATA_ORIGIN <> 'D'
          AND EXISTS
                 (SELECT 1
                    FROM (SELECT INSTITUTION, INSTRUCTION_MODE, EFFDT
                            FROM AMSTG_OWNER.PS_INSTRUCT_MODE T2
                          ...
                          SELECT INSTITUTION, INSTRUCTION_MODE, EFFDT
                            FROM SYSADM.PS_INSTRUCT_MODE@AMSOURCE
                           WHERE (SELECT DELETE_FLG
                                    FROM AMSTG_OWNER.UM_STAGE_JOBS
                                   WHERE TABLE_NAME = 'PS_INSTRUCT_MODE') =
