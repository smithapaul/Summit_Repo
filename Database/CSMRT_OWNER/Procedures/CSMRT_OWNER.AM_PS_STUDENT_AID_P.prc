DROP PROCEDURE CSMRT_OWNER.AM_PS_STUDENT_AID_P
/

--
-- AM_PS_STUDENT_AID_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STUDENT_AID_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STUDENT_AID';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STUDENT_AID@AMSOURCE S)
 where TABLE_NAME = 'PS_STUDENT_AID'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_STUDENT_AID';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_STUDENT_AID';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STUDENT_AID T
using (select /*+ full(S) */
...
from SYSADM.PS_STUDENT_AID@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STUDENT_AID')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STUDENT_AID';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STUDENT_AID';
update AMSTG_OWNER.PS_STUDENT_AID T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR
   from AMSTG_OWNER.PS_STUDENT_AID T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STUDENT_AID') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR
   from SYSADM.PS_STUDENT_AID@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STUDENT_AID') = 'Y'
   ) S
