DROP PROCEDURE CSMRT_OWNER.UM_F_STDNT_TRANSFER_P
/

--
-- UM_F_STDNT_TRANSFER_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."UM_F_STDNT_TRANSFER_P" AUTHID CURRENT_USER IS

------------------------------------------------------------------------
--George Adams
--Loads table                -- UM_F_STDNT_TRANSFER
--V01 12/13/2018             -- srikanth ,pabbu converted to proc from sql scripts

------------------------------------------------------------------------

        strMartId                       Varchar2(50)    := 'CSW';
        strProcessName                  Varchar2(100)   := 'UM_F_STDNT_TRANSFER';
...
strMessage01    := 'Truncating table CSMRT_OWNER.UM_F_STDNT_TRANSFER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlDynamic   := 'truncate table CSMRT_OWNER.UM_F_STDNT_TRANSFER';
...
strMessage01    := 'Disabling Indexes for table CSMRT_OWNER.UM_F_STDNT_TRANSFER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);
COMMON_OWNER.SMT_INDEX.ALL_UNUSABLE('CSMRT_OWNER','UM_F_STDNT_TRANSFER');

--strSqlDynamic   := 'alter table CSMRT_OWNER.UM_F_STDNT_TRANSFER disable constraint PK_UM_F_STDNT_TRANSFER';
--strSqlCommand   := 'SMT_UTILITY.EXECUTE_IMMEDIATE: ' || strSqlDynamic;
--COMMON_OWNER.SMT_UTILITY.EXECUTE_IMMEDIATE
--                (
--                i_SqlStatement          => strSqlDynamic,
...
strMessage01    := 'Inserting data into CSMRT_OWNER.UM_F_STDNT_TRANSFER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'insert into CSMRT_OWNER.UM_F_STDNT_TRANSFER';
insert /*+ append parallel(8) enable_parallel_dml */ INTO UM_F_STDNT_TRANSFER
  with X as (
select /*+ inline parallel(8) */
 ...
 where DATA_ORIGIN <> 'D')
   SELECT /*+ parallel(8) */
     ...
     LEFT OUTER JOIN UM_F_TRANSFER F
       ON T.TERM_SID = F.ARTICULATION_TERM_SID
      AND T.PERSON_SID = F.PERSON_SID
      AND T.SRC_SYS_ID = F.SRC_SYS_ID
...
strMessage01    := '# of UM_F_STDNT_TRANSFER rows inserted: ' || TO_CHAR(intRowCount,'999,999,999,999');
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand := 'SMT_PROCESS_LOG.PROCESS_DETAIL';
COMMON_OWNER.SMT_PROCESS_LOG.PROCESS_DETAIL
        (
                i_TargetTableName   => 'UM_F_STDNT_TRANSFER',
                i_Action            => 'INSERT',
                i_RowCount          => intRowCount
        );

strMessage01    := 'Enabling Indexes for table CSMRT_OWNER.UM_F_STDNT_TRANSFER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

--strSqlDynamic   := 'alter table CSMRT_OWNER.UM_F_STDNT_TRANSFER enable constraint PK_UM_F_STDNT_TRANSFER';
--strSqlCommand   := 'SMT_UTILITY.EXECUTE_IMMEDIATE: ' || strSqlDynamic;
--COMMON_OWNER.SMT_UTILITY.EXECUTE_IMMEDIATE
--                (
--                i_SqlStatement          => strSqlDynamic,
...
COMMON_OWNER.SMT_INDEX.ALL_REBUILD('CSMRT_OWNER','UM_F_STDNT_TRANSFER');
