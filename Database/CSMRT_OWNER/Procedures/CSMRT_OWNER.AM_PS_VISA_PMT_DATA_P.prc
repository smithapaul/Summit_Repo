DROP PROCEDURE CSMRT_OWNER.AM_PS_VISA_PMT_DATA_P
/

--
-- AM_PS_VISA_PMT_DATA_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_VISA_PMT_DATA_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_VISA_PMT_DATA';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_VISA_PMT_DATA@AMSOURCE S)
 where TABLE_NAME = 'PS_VISA_PMT_DATA'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_VISA_PMT_DATA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_VISA_PMT_DATA';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_VISA_PMT_DATA T
using (select /*+ full(S) */
...
from SYSADM.PS_VISA_PMT_DATA@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_VISA_PMT_DATA')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_VISA_PMT_DATA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_VISA_PMT_DATA';
update AMSTG_OWNER.PS_VISA_PMT_DATA T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, DEPENDENT_ID, COUNTRY, VISA_PERMIT_TYPE, EFFDT
   from AMSTG_OWNER.PS_VISA_PMT_DATA T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_VISA_PMT_DATA') = 'Y'
  minus
 select nvl(trim(EMPLID),'-') EMPLID, 
    ...
    nvl(trim(VISA_PERMIT_TYPE),'-') VISA_PERMIT_TYPE,
    EFFDT
   from SYSADM.PS_VISA_PMT_DATA@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_VISA_PMT_DATA') = 'Y' 
   ) S
