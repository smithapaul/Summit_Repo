DROP PROCEDURE CSMRT_OWNER.PS_SSR_STDGRD_ENRL_P
/

--
-- PS_SSR_STDGRD_ENRL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_SSR_STDGRD_ENRL_P"
   AUTHID CURRENT_USER
IS
   /*
...
BEGIN
   ...
   UPDATE CSSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_SSR_STDGRD_ENRL@SASOURCE S)
    WHERE TABLE_NAME = 'PS_SSR_STDGRD_ENRL';
   ...
   strMessage01 := 'Merging data into CSSTG_OWNER.PS_SSR_STDGRD_ENRL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into CSSTG_OWNER.PS_SSR_STDGRD_ENRL';

merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_SSR_STDGRD_ENRL T 
using (select /*+ full(S) */ 
...
from SYSADM.PS_SSR_STDGRD_ENRL@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SSR_STDGRD_ENRL')
 ) S 
 on ( 
...
values ( 
   ...
   strMessage01 := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_SSR_STDGRD_ENRL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on CSSTG_OWNER.PS_SSR_STDGRD_ENRL';


   UPDATE CSSTG_OWNER.PS_SSR_STDGRD_ENRL T
      ...
      AND EXISTS
                 (SELECT 1
                    FROM (SELECT EMPLID, INSTITUTION, ACAD_CAREER, STDNT_CAR_NBR, ACAD_PROG, EXP_GRAD_TERM, DEGREE, STRM, CLASS_NBR
                            FROM CSSTG_OWNER.PS_SSR_STDGRD_ENRL T2
                          ...
                          SELECT EMPLID, INSTITUTION, ACAD_CAREER, STDNT_CAR_NBR, ACAD_PROG, EXP_GRAD_TERM, DEGREE, STRM, CLASS_NBR
                            FROM SYSADM.PS_SSR_STDGRD_ENRL@SASOURCE
                           WHERE (SELECT DELETE_FLG
                                    FROM CSSTG_OWNER.UM_STAGE_JOBS
