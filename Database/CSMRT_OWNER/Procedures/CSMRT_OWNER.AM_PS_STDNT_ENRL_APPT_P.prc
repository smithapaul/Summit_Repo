DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_ENRL_APPT_P
/

--
-- AM_PS_STDNT_ENRL_APPT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_ENRL_APPT_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STDNT_ENRL_APPT';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STDNT_ENRL_APPT@AMSOURCE S)
 where TABLE_NAME = 'PS_STDNT_ENRL_APPT'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_STDNT_ENRL_APPT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_STDNT_ENRL_APPT';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_ENRL_APPT T
using (select /*+ full(S) */
  ...
  from SYSADM.PS_STDNT_ENRL_APPT@AMSOURCE S 
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ENRL_APPT')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_ENRL_APPT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_ENRL_APPT';
update AMSTG_OWNER.PS_STDNT_ENRL_APPT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, INSTITUTION, STRM, SESSION_CODE, SSR_APPT_BLOCK, APPOINTMENT_NBR
   from AMSTG_OWNER.PS_STDNT_ENRL_APPT T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ENRL_APPT') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, INSTITUTION, STRM, SESSION_CODE, SSR_APPT_BLOCK, APPOINTMENT_NBR
   from SYSADM.PS_STDNT_ENRL_APPT@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ENRL_APPT') = 'Y'
   ) S
