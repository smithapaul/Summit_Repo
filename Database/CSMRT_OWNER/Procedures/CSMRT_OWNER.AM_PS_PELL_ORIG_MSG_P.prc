DROP PROCEDURE CSMRT_OWNER.AM_PS_PELL_ORIG_MSG_P
/

--
-- AM_PS_PELL_ORIG_MSG_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_PELL_ORIG_MSG_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_PELL_ORIG_MSG';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_PELL_ORIG_MSG@AMSOURCE S)
 where TABLE_NAME = 'PS_PELL_ORIG_MSG'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_PELL_ORIG_MSG';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_PELL_ORIG_MSG';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_PELL_ORIG_MSG T
using (select /*+ full(S) */
...
PELL_ORIG_ACTN_MSG PELL_ORIG_ACTN_MSG
  from SYSADM.PS_PELL_ORIG_MSG@AMSOURCE S
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_ORIG_MSG')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_PELL_ORIG_MSG';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_PELL_ORIG_MSG';
update AMSTG_OWNER.PS_PELL_ORIG_MSG T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, PELL_ORIG_ID, PELL_ORIG_ACTN_SEQ, PELL_ORIG_MSG_SEQ
   from AMSTG_OWNER.PS_PELL_ORIG_MSG T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_ORIG_MSG') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, PELL_ORIG_ID, PELL_ORIG_ACTN_SEQ, PELL_ORIG_MSG_SEQ
   from SYSADM.PS_PELL_ORIG_MSG@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_ORIG_MSG') = 'Y'
   ) S
