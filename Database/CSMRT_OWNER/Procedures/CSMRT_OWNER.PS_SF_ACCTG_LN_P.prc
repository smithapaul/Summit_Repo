DROP PROCEDURE CSMRT_OWNER.PS_SF_ACCTG_LN_P
/

--
-- PS_SF_ACCTG_LN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_SF_ACCTG_LN_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_SF_ACCTG_LN';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_SF_ACCTG_LN@SASOURCE S)
 where TABLE_NAME = 'PS_SF_ACCTG_LN'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_SF_ACCTG_LN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_SF_ACCTG_LN';
merge /*+ use_hash(S,T) parallel(8) enable_parallel_dml */ into CSSTG_OWNER.PS_SF_ACCTG_LN T
using (select /*+ full(S) */
...
from SYSADM.PS_SF_ACCTG_LN@SASOURCE S
--where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SF_ACCTG_LN') ) S
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_SF_ACCTG_LN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_SF_ACCTG_LN';
update /*+ parallel(8) enable_parallel_dml */ CSSTG_OWNER.PS_SF_ACCTG_LN T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select RUN_DT, SEQNUM, SF_LINE_NBR
   from CSSTG_OWNER.PS_SF_ACCTG_LN T2
--  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SF_ACCTG_LN') = 'Y'
  minus
 select RUN_DT, SEQNUM, SF_LINE_NBR
   from SYSADM.PS_SF_ACCTG_LN@SASOURCE S2
--  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SF_ACCTG_LN') = 'Y'
   ) S
