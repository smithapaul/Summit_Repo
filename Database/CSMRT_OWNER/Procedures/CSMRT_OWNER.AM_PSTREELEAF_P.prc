DROP PROCEDURE CSMRT_OWNER.AM_PSTREELEAF_P
/

--
-- AM_PSTREELEAF_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PSTREELEAF_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PSTREELEAF@AMSOURCE S)
    WHERE TABLE_NAME = 'PSTREELEAF';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PSTREELEAF';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PSTREELEAF';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PSTREELEAF T 
using (select /*+ full(S) */ 
...
from SYSADM.PSTREELEAF@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSTREELEAF') ) S 
 on (
	T.SETID = S.SETID and
...
strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PSTREELEAF';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PSTREELEAF';
update AMSTG_OWNER.PSTREELEAF T
  set T.DATA_ORIGIN = 'D',
      T.LASTUPD_EW_DTTM = SYSDATE
...
(select SETID, SETCNTRLVALUE, TREE_NAME, EFFDT, TREE_NODE_NUM, RANGE_FROM, RANGE_TO, TREE_BRANCH
   from AMSTG_OWNER.PSTREELEAF T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSTREELEAF') = 'Y'
  minus
 select nvl(trim(SETID),'-') SETID,
	    ...
	    nvl(trim(RANGE_TO),'-') RANGE_TO,
	    nvl(trim(TREE_BRANCH),'-') TREE_BRANCH
   from SYSADM.PSTREELEAF@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSTREELEAF') = 'Y' 
   ) S
