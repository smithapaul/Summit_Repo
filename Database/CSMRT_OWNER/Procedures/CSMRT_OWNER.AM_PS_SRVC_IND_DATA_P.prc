DROP PROCEDURE CSMRT_OWNER.AM_PS_SRVC_IND_DATA_P
/

--
-- AM_PS_SRVC_IND_DATA_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_SRVC_IND_DATA_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_SRVC_IND_DATA';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_SRVC_IND_DATA@AMSOURCE S)
 where TABLE_NAME = 'PS_SRVC_IND_DATA'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_SRVC_IND_DATA';
...
insert /*+ append */  into AMSTG_OWNER.PS_T_SRVC_IND_DATA
select /*+ full(S) */
...
from SYSADM.PS_SRVC_IND_DATA@AMSOURCE S 
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_SRVC_IND_DATA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_SRVC_IND_DATA';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_SRVC_IND_DATA T
using (select /*+ full(S) */
    nvl(trim(EMPLID),'-') EMPLID, 
    SRVC_IND_DTTM, 
    nvl(trim(OPRID),'-') OPRID, 
    ...
    nvl(trim(RELEASE_PROCESS),'-') RELEASE_PROCESS, 
    COMM_COMMENTS
from AMSTG_OWNER.PS_T_SRVC_IND_DATA S 
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SRVC_IND_DATA') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_SRVC_IND_DATA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_SRVC_IND_DATA';
update AMSTG_OWNER.PS_SRVC_IND_DATA T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, SRVC_IND_DTTM
   from AMSTG_OWNER.PS_SRVC_IND_DATA T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SRVC_IND_DATA') = 'Y'
  minus
 select EMPLID, SRVC_IND_DTTM
   from SYSADM.PS_SRVC_IND_DATA@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SRVC_IND_DATA') = 'Y'
   ) S
