DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_TEST_COMP_P
/

--
-- AM_PS_STDNT_TEST_COMP_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_TEST_COMP_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STDNT_TEST_COMP';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STDNT_TEST_COMP@AMSOURCE S)
 where TABLE_NAME = 'PS_STDNT_TEST_COMP'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_STDNT_TEST_COMP';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_STDNT_TEST_COMP';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_TEST_COMP T
using (select /*+ full(S) */
...
from SYSADM.PS_STDNT_TEST_COMP@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_TEST_COMP') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_TEST_COMP';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_TEST_COMP';
update AMSTG_OWNER.PS_STDNT_TEST_COMP T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, TEST_ID, TEST_COMPONENT, TEST_DT, LS_DATA_SOURCE
   from AMSTG_OWNER.PS_STDNT_TEST_COMP T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_TEST_COMP') = 'Y'
  minus
 select EMPLID, TEST_ID, TEST_COMPONENT, (case when TEST_DT < '01-JAN-1800' then to_date('01-JAN-1800') else TEST_DT end) TEST_DT, LS_DATA_SOURCE
   from SYSADM.PS_STDNT_TEST_COMP@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_TEST_COMP') = 'Y'
   ) S
