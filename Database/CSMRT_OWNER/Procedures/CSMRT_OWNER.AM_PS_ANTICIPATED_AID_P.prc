DROP PROCEDURE CSMRT_OWNER.AM_PS_ANTICIPATED_AID_P
/

--
-- AM_PS_ANTICIPATED_AID_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ANTICIPATED_AID_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ANTICIPATED_AID';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ANTICIPATED_AID@AMSOURCE S)
 where TABLE_NAME = 'PS_ANTICIPATED_AID'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ANTICIPATED_AID';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ANTICIPATED_AID';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ANTICIPATED_AID T
using (select /*+ full(S) */
...
--    AS_OF_DTTM AS_OF_DTTM,
  ...
  from SYSADM.PS_ANTICIPATED_AID@AMSOURCE S 
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ANTICIPATED_AID')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ANTICIPATED_AID';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ANTICIPATED_AID';
update AMSTG_OWNER.PS_ANTICIPATED_AID T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, ITEM_TYPE, ACAD_CAREER, DISBURSEMENT_PLAN, DISBURSEMENT_ID, AS_OF_DTTM
   from AMSTG_OWNER.PS_ANTICIPATED_AID T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ANTICIPATED_AID') = 'Y'
  minus
 select EMPLID, INSTITUTION, nvl(trim(AID_YEAR),'-') AID_YEAR, nvl(trim(ITEM_TYPE),'-') ITEM_TYPE, nvl(trim(ACAD_CAREER),'-') ACAD_CAREER, DISBURSEMENT_PLAN, DISBURSEMENT_ID, AS_OF_DTTM
   from SYSADM.PS_ANTICIPATED_AID@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ANTICIPATED_AID') = 'Y'
   ) S
