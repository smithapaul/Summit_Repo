DROP PROCEDURE CSMRT_OWNER.AM_PS_SSR_STDGRD_HIST_P
/

--
-- AM_PS_SSR_STDGRD_HIST_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_SSR_STDGRD_HIST_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_SSR_STDGRD_HIST@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_SSR_STDGRD_HIST';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_SSR_STDGRD_HIST';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_SSR_STDGRD_HIST';

merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_SSR_STDGRD_HIST T 
using (select /*+ full(S) */ 
...
from SYSADM.PS_SSR_STDGRD_HIST@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SSR_STDGRD_HIST') 
...
values ( 
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_SSR_STDGRD_HIST';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_SSR_STDGRD_HIST';


   UPDATE AMSTG_OWNER.PS_SSR_STDGRD_HIST T
      ...
      AND EXISTS
                 (SELECT 1
                    FROM (SELECT EMPLID, INSTITUTION, ACAD_CAREER, STDNT_CAR_NBR, ACAD_PROG, EXP_GRAD_TERM, DEGREE, SSR_GRAD_REV_DTTM
                            FROM AMSTG_OWNER.PS_SSR_STDGRD_HIST T2
                          ...
                          SELECT nvl(trim(EMPLID),'-') EMPLID,
								...
								nvl(trim(DEGREE),'-') DEGREE,
								SSR_GRAD_REV_DTTM 
                            FROM SYSADM.PS_SSR_STDGRD_HIST@AMSOURCE
                           WHERE (SELECT DELETE_FLG
                                    FROM AMSTG_OWNER.UM_STAGE_JOBS
