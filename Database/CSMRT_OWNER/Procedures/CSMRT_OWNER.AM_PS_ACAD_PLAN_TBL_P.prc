DROP PROCEDURE CSMRT_OWNER.AM_PS_ACAD_PLAN_TBL_P
/

--
-- AM_PS_ACAD_PLAN_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_ACAD_PLAN_TBL_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ACAD_PLAN_TBL';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ACAD_PLAN_TBL@AMSOURCE S)
 where TABLE_NAME = 'PS_ACAD_PLAN_TBL'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_ACAD_PLAN_TBL';
...
INSERT /*+ append */
      INTO  AMSTG_OWNER.PS_T_ACAD_PLAN_TBL
   SELECT /*+ full(S) */
        INSTITUTION,
         ACAD_PLAN,
        EFFDT,         
...
from SYSADM.PS_ACAD_PLAN_TBL@AMSOURCE S
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ACAD_PLAN_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ACAD_PLAN_TBL';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ACAD_PLAN_TBL T
using (select /*+ full(S) */
...
DESCRLONG DESCRLONG
from AMSTG_OWNER.PS_T_ACAD_PLAN_TBL S
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_PLAN_TBL') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_PLAN_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_PLAN_TBL';
update AMSTG_OWNER.PS_ACAD_PLAN_TBL T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select INSTITUTION, ACAD_PLAN, EFFDT
   from AMSTG_OWNER.PS_ACAD_PLAN_TBL T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_PLAN_TBL') = 'Y'
  minus
 select INSTITUTION, ACAD_PLAN, EFFDT
   from SYSADM.PS_ACAD_PLAN_TBL@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_PLAN_TBL') = 'Y'
   ) S
