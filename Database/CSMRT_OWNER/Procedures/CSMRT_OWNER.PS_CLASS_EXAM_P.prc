DROP PROCEDURE CSMRT_OWNER.PS_CLASS_EXAM_P
/

--
-- PS_CLASS_EXAM_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_CLASS_EXAM_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_CLASS_EXAM';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_CLASS_EXAM@SASOURCE S)
 where TABLE_NAME = 'PS_CLASS_EXAM'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_CLASS_EXAM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_CLASS_EXAM';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_CLASS_EXAM T
using (select /*+ full(S) */
...
from SYSADM.PS_CLASS_EXAM@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_EXAM') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_CLASS_EXAM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_CLASS_EXAM';
update CSSTG_OWNER.PS_CLASS_EXAM T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION, CLASS_EXAM_SEQ
   from CSSTG_OWNER.PS_CLASS_EXAM T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_EXAM') = 'Y'
  minus
 select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION, CLASS_EXAM_SEQ
   from SYSADM.PS_CLASS_EXAM@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_EXAM') = 'Y' 
   ) S
