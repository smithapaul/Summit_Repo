DROP PROCEDURE CSMRT_OWNER.AM_PS_ACAD_LEVEL_TBL_P
/

--
-- AM_PS_ACAD_LEVEL_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ACAD_LEVEL_TBL_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ACAD_LEVEL_TBL';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ACAD_LEVEL_TBL@AMSOURCE S)
 where TABLE_NAME = 'PS_ACAD_LEVEL_TBL'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ACAD_LEVEL_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ACAD_LEVEL_TBL';

merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ACAD_LEVEL_TBL T
using (select /*+ full(S) */
...
from SYSADM.PS_ACAD_LEVEL_TBL@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_LEVEL_TBL') ) S
   on (
T.SETID = S.SETID and
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_LEVEL_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_LEVEL_TBL';

    update AMSTG_OWNER.PS_ACAD_LEVEL_TBL T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select SETID, LEVEL_LOAD_RULE, EFFDT, UNT_CUM_TOTAL, ACADEMIC_LEVEL
   from AMSTG_OWNER.PS_ACAD_LEVEL_TBL T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_LEVEL_TBL') = 'Y'
  minus
 select SETID, LEVEL_LOAD_RULE, EFFDT, UNT_CUM_TOTAL, ACADEMIC_LEVEL
   from SYSADM.PS_ACAD_LEVEL_TBL@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_LEVEL_TBL') = 'Y' 
-- AND EMPLID <>'00386824'
   ) S
