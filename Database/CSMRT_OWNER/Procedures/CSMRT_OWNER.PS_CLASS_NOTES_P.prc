DROP PROCEDURE CSMRT_OWNER.PS_CLASS_NOTES_P
/

--
-- PS_CLASS_NOTES_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_CLASS_NOTES_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_CLASS_NOTES';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_CLASS_NOTES@SASOURCE S)
 where TABLE_NAME = 'PS_CLASS_NOTES'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_CLASS_NOTES';
...
INSERT /*+ append */
      INTO  CSSTG_OWNER.PS_T_CLASS_NOTES
   SELECT /*+ full(S) */
         CRSE_ID,
          CRSE_OFFER_NBR,
          STRM,
     ...
     FROM SYSADM.PS_CLASS_NOTES@SASOURCE;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_CLASS_NOTES';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_CLASS_NOTES';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_CLASS_NOTES T
using (select /*+ full(S) */
...
DESCRLONG DESCRLONG
from CSSTG_OWNER.PS_T_CLASS_NOTES S
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_NOTES') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_CLASS_NOTES';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_CLASS_NOTES';
update CSSTG_OWNER.PS_CLASS_NOTES T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION, CLASS_NOTES_SEQ
   from CSSTG_OWNER.PS_CLASS_NOTES T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_NOTES') = 'Y'
  minus
 select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION, CLASS_NOTES_SEQ
   from SYSADM.PS_CLASS_NOTES@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_NOTES') = 'Y'
   ) S
