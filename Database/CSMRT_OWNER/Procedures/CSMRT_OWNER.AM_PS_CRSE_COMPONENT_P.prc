DROP PROCEDURE CSMRT_OWNER.AM_PS_CRSE_COMPONENT_P
/

--
-- AM_PS_CRSE_COMPONENT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_CRSE_COMPONENT_P" IS
   ...
   strProcessName     VARCHAR2 (100) := 'AM_PS_CRSE_COMPONENT';
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_CRSE_COMPONENT@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_CRSE_COMPONENT';

   strSqlCommand := 'commit';
   COMMIT;

   strMessage01 := 'Merging data into AMSTG_OWNER.PS_CRSE_COMPONENT';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_CRSE_COMPONENT';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_CRSE_COMPONENT T
using (select /*+ full(S) */
...
from SYSADM.PS_CRSE_COMPONENT@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CRSE_COMPONENT') ) S
   on (
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_CRSE_COMPONENT';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_CRSE_COMPONENT';

update AMSTG_OWNER.PS_CRSE_COMPONENT T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select CRSE_ID, EFFDT, SSR_COMPONENT
   from AMSTG_OWNER.PS_CRSE_COMPONENT T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CRSE_COMPONENT') = 'Y'
  minus
 select CRSE_ID, EFFDT, SSR_COMPONENT
   from SYSADM.PS_CRSE_COMPONENT@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CRSE_COMPONENT') = 'Y' 
   ) S
