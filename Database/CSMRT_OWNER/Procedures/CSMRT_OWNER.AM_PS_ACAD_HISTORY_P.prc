DROP PROCEDURE CSMRT_OWNER.AM_PS_ACAD_HISTORY_P
/

--
-- AM_PS_ACAD_HISTORY_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ACAD_HISTORY_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_ACAD_HISTORY@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_ACAD_HISTORY';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_ACAD_HISTORY';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_ACAD_HISTORY';

merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ACAD_HISTORY T
using (select /*+ full(S) */
nvl(trim(EMPLID),'-') EMPLID,
nvl(trim(EXT_ORG_ID),'-') EXT_ORG_ID,
nvl(trim(CHKLST_ITM_UPDT),'-') CHKLST_ITM_UPDT
from SYSADM.PS_ACAD_HISTORY@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_HISTORY')
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_HISTORY';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_HISTORY';


update AMSTG_OWNER.PS_ACAD_HISTORY T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, EXT_ORG_ID
   from AMSTG_OWNER.PS_ACAD_HISTORY T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_HISTORY') = 'Y'
  minus
 select EMPLID, EXT_ORG_ID
   from SYSADM.PS_ACAD_HISTORY@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_HISTORY') = 'Y' 
   ) S
