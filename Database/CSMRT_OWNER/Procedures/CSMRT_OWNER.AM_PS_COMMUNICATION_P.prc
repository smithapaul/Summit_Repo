DROP PROCEDURE CSMRT_OWNER.AM_PS_COMMUNICATION_P
/

--
-- AM_PS_COMMUNICATION_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_COMMUNICATION_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_COMMUNICATION';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_COMMUNICATION@AMSOURCE S)
 where TABLE_NAME = 'PS_COMMUNICATION'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_COMMUNICATION';
...
insert /*+ append */  into AMSTG_OWNER.PS_T_COMMUNICATION
select /*+ full(S) */
  ...
  from SYSADM.PS_COMMUNICATION@AMSOURCE S
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_COMMUNICATION';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_COMMUNICATION';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_COMMUNICATION T
using (select /*+ full(S) */
    nvl(trim(COMMON_ID),'-') COMMON_ID,
    nvl(SEQ_3C,0) SEQ_3C,
    nvl(trim(SA_ID_TYPE),'-') SA_ID_TYPE,
...
from AMSTG_OWNER.PS_T_COMMUNICATION S
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_COMMUNICATION') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_COMMUNICATION';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_COMMUNICATION';
update AMSTG_OWNER.PS_COMMUNICATION T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select COMMON_ID, SEQ_3C
   from AMSTG_OWNER.PS_COMMUNICATION T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_COMMUNICATION') = 'Y'
  minus
 select COMMON_ID, SEQ_3C
   from SYSADM.PS_COMMUNICATION@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_COMMUNICATION') = 'Y'
   ) S
