DROP PROCEDURE CSMRT_OWNER.AM_PS_TRNS_CRSE_SCH_P
/

--
-- AM_PS_TRNS_CRSE_SCH_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_TRNS_CRSE_SCH_P IS
...
--VXX    07/06/2021,            Kieu ,Srikanth - Added EMPLID or COMMON_ID additional filter log
------------------------------------------------------------------------

        strMartId                       Varchar2(50)    := 'CSW';
        strProcessName                  Varchar2(100)   := 'AM_PS_TRNS_CRSE_SCH';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_TRNS_CRSE_SCH@AMSOURCE S)
 where TABLE_NAME = 'PS_TRNS_CRSE_SCH'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_TRNS_CRSE_SCH';
...
insert /*+ append */  into AMSTG_OWNER.PS_T_TRNS_CRSE_SCH
select /*+ full(S) */
...
from SYSADM.PS_TRNS_CRSE_SCH@AMSOURCE S 
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_TRNS_CRSE_SCH';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_TRNS_CRSE_SCH';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_TRNS_CRSE_SCH T
using (select /*+ full(S) */
    nvl(trim(EMPLID),'-') EMPLID, 
    nvl(trim(ACAD_CAREER),'-') ACAD_CAREER, 
    nvl(trim(INSTITUTION),'-') INSTITUTION, 
    ...
    nvl(trim(INCLUDE_IN_GPA),'-') INCLUDE_IN_GPA, 
    COMMENTS COMMENTS
from AMSTG_OWNER.PS_T_TRNS_CRSE_SCH S 
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_CRSE_SCH') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_TRNS_CRSE_SCH';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_TRNS_CRSE_SCH';
update AMSTG_OWNER.PS_TRNS_CRSE_SCH T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, INSTITUTION, MODEL_NBR
   from AMSTG_OWNER.PS_TRNS_CRSE_SCH T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_CRSE_SCH') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, INSTITUTION, MODEL_NBR
   from SYSADM.PS_TRNS_CRSE_SCH@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_CRSE_SCH') = 'Y'
   ) S
