DROP PROCEDURE CSMRT_OWNER.AM_PS_SSS_CRSE_PLNR_P
/

--
-- AM_PS_SSS_CRSE_PLNR_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_SSS_CRSE_PLNR_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_SSS_CRSE_PLNR';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_SSS_CRSE_PLNR@AMSOURCE S)
 where TABLE_NAME = 'PS_SSS_CRSE_PLNR'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_SSS_CRSE_PLNR';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_SSS_CRSE_PLNR';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_SSS_CRSE_PLNR T
using (select /*+ full(S) */
  ...
  from SYSADM.PS_SSS_CRSE_PLNR@AMSOURCE S 
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SSS_CRSE_PLNR')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_SSS_CRSE_PLNR';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_SSS_CRSE_PLNR';
update AMSTG_OWNER.PS_SSS_CRSE_PLNR T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, INSTITUTION, SAA_PLNR_SEQ, SAA_PLNR_CRSE_SEQ
   from AMSTG_OWNER.PS_SSS_CRSE_PLNR T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SSS_CRSE_PLNR') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, INSTITUTION, SAA_PLNR_SEQ, SAA_PLNR_CRSE_SEQ
   from SYSADM.PS_SSS_CRSE_PLNR@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SSS_CRSE_PLNR') = 'Y'
   ) S
