DROP PROCEDURE CSMRT_OWNER.AM_PS_SCTN_CMBND_P
/

--
-- AM_PS_SCTN_CMBND_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_SCTN_CMBND_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_SCTN_CMBND@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_SCTN_CMBND';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_SCTN_CMBND';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_SCTN_CMBND';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_SCTN_CMBND T
using (select /*+ full(S) */
...
from SYSADM.PS_SCTN_CMBND@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SCTN_CMBND') ) S
   on (
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_SCTN_CMBND';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_SCTN_CMBND';

update AMSTG_OWNER.PS_SCTN_CMBND T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select INSTITUTION, STRM, SESSION_CODE, CLASS_NBR
   from AMSTG_OWNER.PS_SCTN_CMBND T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SCTN_CMBND') = 'Y'
  minus
 select INSTITUTION, STRM, SESSION_CODE, CLASS_NBR
   from SYSADM.PS_SCTN_CMBND@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SCTN_CMBND') = 'Y' 
   ) S
