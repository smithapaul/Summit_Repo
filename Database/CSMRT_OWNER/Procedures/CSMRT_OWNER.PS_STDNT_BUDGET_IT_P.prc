DROP PROCEDURE CSMRT_OWNER.PS_STDNT_BUDGET_IT_P
/

--
-- PS_STDNT_BUDGET_IT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_STDNT_BUDGET_IT_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_STDNT_BUDGET_IT';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STDNT_BUDGET_IT@SASOURCE S)
 where TABLE_NAME = 'PS_STDNT_BUDGET_IT'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_STDNT_BUDGET_IT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_STDNT_BUDGET_IT';
merge /*+ use_hash(S,T) parallel(8) enable_parallel_dml */ into CSSTG_OWNER.PS_STDNT_BUDGET_IT T
using (select /*+ full(S) */
  ...
  from SYSADM.PS_STDNT_BUDGET_IT@SASOURCE S
-- where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_BUDGET_IT')
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_STDNT_BUDGET_IT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_STDNT_BUDGET_IT';
update /*+ parallel(8) enable_parallel_dml */ CSSTG_OWNER.PS_STDNT_BUDGET_IT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, ACAD_CAREER, STRM, EFFDT, EFFSEQ, BGT_ITEM_CATEGORY
   from CSSTG_OWNER.PS_STDNT_BUDGET_IT T2
--  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_BUDGET_IT') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, ACAD_CAREER, STRM, EFFDT, EFFSEQ, BGT_ITEM_CATEGORY
   from SYSADM.PS_STDNT_BUDGET_IT@SASOURCE S2
--  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_BUDGET_IT') = 'Y'
   ) S
