DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_CAR_TERM_P
/

--
-- AM_PS_STDNT_CAR_TERM_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_CAR_TERM_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STDNT_CAR_TERM';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STDNT_CAR_TERM@AMSOURCE S)
 where TABLE_NAME = 'PS_STDNT_CAR_TERM'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_STDNT_CAR_TERM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_STDNT_CAR_TERM';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_CAR_TERM T
using (select /*+ full(S) */
...
from SYSADM.PS_STDNT_CAR_TERM@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_CAR_TERM') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_CAR_TERM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_CAR_TERM';
update AMSTG_OWNER.PS_STDNT_CAR_TERM T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, INSTITUTION, STRM
   from AMSTG_OWNER.PS_STDNT_CAR_TERM T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_CAR_TERM') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, INSTITUTION, STRM
   from SYSADM.PS_STDNT_CAR_TERM@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_CAR_TERM') = 'Y'
   ) S
