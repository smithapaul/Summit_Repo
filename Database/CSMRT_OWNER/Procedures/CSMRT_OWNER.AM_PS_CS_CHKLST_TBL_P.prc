DROP PROCEDURE CSMRT_OWNER.AM_PS_CS_CHKLST_TBL_P
/

--
-- AM_PS_CS_CHKLST_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_CS_CHKLST_TBL_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_CS_CHKLST_TBL@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_CS_CHKLST_TBL';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_CS_CHKLST_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

strSqlCommand := 'merge into AMSTG_OWNER.PS_CS_CHKLST_TBL';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_CS_CHKLST_TBL T
using (select /*+ full(S) */
...
from SYSADM.PS_CS_CHKLST_TBL@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CS_CHKLST_TBL') ) S
 on ( 
...
;
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_CS_CHKLST_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_CS_CHKLST_TBL';
update AMSTG_OWNER.PS_CS_CHKLST_TBL T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select INSTITUTION, CHECKLIST_CD, EFFDT
   from AMSTG_OWNER.PS_CS_CHKLST_TBL T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CS_CHKLST_TBL') = 'Y'
  minus
 select INSTITUTION, CHECKLIST_CD, EFFDT
   from SYSADM.PS_CS_CHKLST_TBL@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CS_CHKLST_TBL') = 'Y'
   ) S
