DROP PROCEDURE CSMRT_OWNER.AM_PS_SFA_SAP_STDNT_P
/

--
-- AM_PS_SFA_SAP_STDNT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_SFA_SAP_STDNT_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_SFA_SAP_STDNT';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_SFA_SAP_STDNT@AMSOURCE S)
 where TABLE_NAME = 'PS_SFA_SAP_STDNT'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_SFA_SAP_STDNT';
...
insert /*+ append */  into AMSTG_OWNER.PS_T_SFA_SAP_STDNT
select /*+ full(S) */
...
to_number(ORA_ROWSCN) SRC_SCN
  from SYSADM.PS_SFA_SAP_STDNT@AMSOURCE S
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_SFA_SAP_STDNT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_SFA_SAP_STDNT';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_SFA_SAP_STDNT T
using (select /*+ full(S) */
nvl(trim(EMPLID),'-') EMPLID,
nvl(trim(INSTITUTION),'-') INSTITUTION,
nvl(trim(ACAD_CAREER),'-') ACAD_CAREER,
...
from AMSTG_OWNER.PS_T_SFA_SAP_STDNT S
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SFA_SAP_STDNT') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_SFA_SAP_STDNT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_SFA_SAP_STDNT';
update AMSTG_OWNER.PS_SFA_SAP_STDNT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, ACAD_CAREER, AID_YEAR, STRM, PROCESS_DTTM
   from AMSTG_OWNER.PS_SFA_SAP_STDNT T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SFA_SAP_STDNT') = 'Y'
  minus
 select nvl(trim(EMPLID),'-') EMPLID,
    ...
    nvl(trim(STRM),'-') STRM,
    PROCESS_DTTM
   from SYSADM.PS_SFA_SAP_STDNT@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SFA_SAP_STDNT') = 'Y'
   ) S
