DROP PROCEDURE CSMRT_OWNER.PS_GRADE_ROSTER_P
/

--
-- PS_GRADE_ROSTER_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_GRADE_ROSTER_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_GRADE_ROSTER';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_GRADE_ROSTER@SASOURCE S)
 where TABLE_NAME = 'PS_GRADE_ROSTER'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_GRADE_ROSTER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_GRADE_ROSTER';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_GRADE_ROSTER T
using (select /*+ full(S) */
    nvl(trim(STRM),'-') STRM,
...
from SYSADM.PS_GRADE_ROSTER@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRADE_ROSTER') 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_GRADE_ROSTER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_GRADE_ROSTER';
update CSSTG_OWNER.PS_GRADE_ROSTER T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select STRM, CLASS_NBR, GRD_RSTR_TYPE_SEQ, EMPLID, ACAD_CAREER
   from CSSTG_OWNER.PS_GRADE_ROSTER T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRADE_ROSTER') = 'Y'
  minus
 select STRM, CLASS_NBR, GRD_RSTR_TYPE_SEQ, EMPLID, ACAD_CAREER
   from SYSADM.PS_GRADE_ROSTER@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRADE_ROSTER') = 'Y'
   ) S
 where T.STRM = S.STRM
