DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_ADVR_HIST_P
/

--
-- AM_PS_STDNT_ADVR_HIST_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_ADVR_HIST_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_STDNT_ADVR_HIST@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_STDNT_ADVR_HIST';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_STDNT_ADVR_HIST';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_STDNT_ADVR_HIST';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_ADVR_HIST T
using (select /*+ full(S) */
...
from SYSADM.PS_STDNT_ADVR_HIST@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ADVR_HIST') 
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_ADVR_HIST';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_ADVR_HIST';

update AMSTG_OWNER.PS_STDNT_ADVR_HIST T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, EFFDT, ADVISOR_ROLE, STDNT_ADVISOR_NBR
   from AMSTG_OWNER.PS_STDNT_ADVR_HIST T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ADVR_HIST') = 'Y'
  minus
 select EMPLID, INSTITUTION, EFFDT, ADVISOR_ROLE, STDNT_ADVISOR_NBR
   from SYSADM.PS_STDNT_ADVR_HIST@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ADVR_HIST') = 'Y' 
   ) S
