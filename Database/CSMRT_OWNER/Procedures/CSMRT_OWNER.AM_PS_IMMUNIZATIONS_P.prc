DROP PROCEDURE CSMRT_OWNER.AM_PS_IMMUNIZATIONS_P
/

--
-- AM_PS_IMMUNIZATIONS_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_IMMUNIZATIONS_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_IMMUNIZATIONS';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_IMMUNIZATIONS@AMSOURCE S)
 where TABLE_NAME = 'PS_IMMUNIZATIONS'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_IMMUNIZATIONS';
...
insert /*+ append */  into AMSTG_OWNER.PS_T_IMMUNIZATIONS
select /*+ full(S) */
    EMPLID, 
    IMMUNIZATION, 
    IMMUN_SEQ, 
...
from SYSADM.PS_IMMUNIZATIONS@AMSOURCE S
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_IMMUNIZATIONS';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_IMMUNIZATIONS';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_IMMUNIZATIONS T
using (select /*+ full(S) */
    ...
    to_char(substr(trim(COMMENTS), 1, 4000)) COMMENTS
from AMSTG_OWNER.PS_T_IMMUNIZATIONS S 
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_IMMUNIZATIONS') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_IMMUNIZATIONS';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_IMMUNIZATIONS';
update AMSTG_OWNER.PS_IMMUNIZATIONS T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, IMMUNIZATION, IMMUN_SEQ
   from AMSTG_OWNER.PS_IMMUNIZATIONS T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_IMMUNIZATIONS') = 'Y'
  minus
 select EMPLID, IMMUNIZATION, IMMUN_SEQ
   from SYSADM.PS_IMMUNIZATIONS@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_IMMUNIZATIONS') = 'Y'
   ) S
