DROP PROCEDURE CSMRT_OWNER.AM_PS_CLASS_PRMSN_P
/

--
-- AM_PS_CLASS_PRMSN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_CLASS_PRMSN_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_CLASS_PRMSN';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_CLASS_PRMSN@AMSOURCE S)
 where TABLE_NAME = 'PS_CLASS_PRMSN'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_CLASS_PRMSN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_CLASS_PRMSN';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_CLASS_PRMSN T
using (select /*+ full(S) */
...
from SYSADM.PS_CLASS_PRMSN@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_PRMSN') ) S
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_CLASS_PRMSN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_CLASS_PRMSN';
update AMSTG_OWNER.PS_CLASS_PRMSN T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION, PERMISSION_TYPE, CLASS_PRMSN_SEQ
   from AMSTG_OWNER.PS_CLASS_PRMSN T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_PRMSN') = 'Y'
  minus
 select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION, PERMISSION_TYPE, CLASS_PRMSN_SEQ
   from SYSADM.PS_CLASS_PRMSN@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_PRMSN') = 'Y'
   ) S
