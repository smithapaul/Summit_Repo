DROP PROCEDURE CSMRT_OWNER.AM_PS_GRADE_RSTR_TYPE_P
/

--
-- AM_PS_GRADE_RSTR_TYPE_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_GRADE_RSTR_TYPE_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_GRADE_RSTR_TYPE@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_GRADE_RSTR_TYPE';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_GRADE_RSTR_TYPE';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_GRADE_RSTR_TYPE';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_GRADE_RSTR_TYPE T
using (select /*+ full(S) */
nvl(trim(STRM),'-') STRM,
...
from SYSADM.PS_GRADE_RSTR_TYPE@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRADE_RSTR_TYPE') ) S
   on (
T.STRM = S.STRM and
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_GRADE_RSTR_TYPE';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_GRADE_RSTR_TYPE';

update AMSTG_OWNER.PS_GRADE_RSTR_TYPE T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select STRM, CLASS_NBR, GRD_RSTR_TYPE_SEQ
   from AMSTG_OWNER.PS_GRADE_RSTR_TYPE T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRADE_RSTR_TYPE') = 'Y'
  minus
 select STRM, CLASS_NBR, GRD_RSTR_TYPE_SEQ
   from SYSADM.PS_GRADE_RSTR_TYPE@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRADE_RSTR_TYPE') = 'Y' 
   ) S
