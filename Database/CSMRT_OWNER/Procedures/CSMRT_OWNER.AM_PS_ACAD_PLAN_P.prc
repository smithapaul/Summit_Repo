DROP PROCEDURE CSMRT_OWNER.AM_PS_ACAD_PLAN_P
/

--
-- AM_PS_ACAD_PLAN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ACAD_PLAN_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_ACAD_PLAN@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_ACAD_PLAN';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_ACAD_PLAN';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_ACAD_PLAN';

merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ACAD_PLAN T
using (select /*+ full(S) */
...
from SYSADM.PS_ACAD_PLAN@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_PLAN') 
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_PLAN';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_PLAN';


update AMSTG_OWNER.PS_ACAD_PLAN T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, EFFDT, EFFSEQ, ACAD_PLAN
   from AMSTG_OWNER.PS_ACAD_PLAN T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_PLAN') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, EFFDT, EFFSEQ, ACAD_PLAN
   from SYSADM.PS_ACAD_PLAN@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_PLAN') = 'Y' 
   ) S
