DROP PROCEDURE CSMRT_OWNER.AM_PS_ADM_APPL_DATA_P
/

--
-- AM_PS_ADM_APPL_DATA_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ADM_APPL_DATA_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ADM_APPL_DATA';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ADM_APPL_DATA@AMSOURCE S)
 where TABLE_NAME = 'PS_ADM_APPL_DATA'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ADM_APPL_DATA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ADM_APPL_DATA';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ADM_APPL_DATA T
using (select /*+ full(S) */
...
--    nvl(trim(R.UM_RA_TA_INTEREST),'-') UM_RA_TA_INTEREST
    '-' UM_RA_TA_INTEREST
FROM SYSADM.PS_ADM_APPL_DATA@AMSOURCE S
--LEFT OUTER JOIN SYSADM.PS_UM_RA_TA@AMSOURCE R     -- Jan 2022 
--    ON S.EMPLID = R.EMPLID
--   AND S.ACAD_CAREER = R.ACAD_CAREER
--   AND S.STDNT_CAR_NBR = R.STDNT_CAR_NBR
--   AND S.ADM_APPL_NBR = R.ADM_APPL_NBR
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_DATA')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ADM_APPL_DATA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ADM_APPL_DATA';
update AMSTG_OWNER.PS_ADM_APPL_DATA T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR
   from AMSTG_OWNER.PS_ADM_APPL_DATA T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_DATA') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR
   from SYSADM.PS_ADM_APPL_DATA@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_DATA') = 'Y'
   ) S
