DROP PROCEDURE CSMRT_OWNER.PS_DISB_SPLIT_CD_P
/

--
-- PS_DISB_SPLIT_CD_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_DISB_SPLIT_CD_P"
   AUTHID CURRENT_USER
IS
   /*
...
BEGIN
   ...
   UPDATE CSSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_DISB_SPLIT_CD@SASOURCE S)
    WHERE TABLE_NAME = 'PS_DISB_SPLIT_CD';
   ...
   strMessage01 := 'Merging data into CSSTG_OWNER.PS_DISB_SPLIT_CD';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into CSSTG_OWNER.PS_DISB_SPLIT_CD';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_DISB_SPLIT_CD T
using (select /*+ full(S) */
...
from SYSADM.PS_DISB_SPLIT_CD@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_DISB_SPLIT_CD') ) S
 on ( 
...
values (
   ...
   strMessage01 := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_DISB_SPLIT_CD';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on CSSTG_OWNER.PS_DISB_SPLIT_CD';
update CSSTG_OWNER.PS_DISB_SPLIT_CD T
   set T.DATA_ORIGIN = 'D'
 where T.DATA_ORIGIN <> 'D'
   and exists 
(select 1 from
(select INSTITUTION, AID_YEAR, ACAD_CAREER, DISBURSEMENT_PLAN, SPLIT_CODE
   from CSSTG_OWNER.PS_DISB_SPLIT_CD T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_DISB_SPLIT_CD') = 'Y'
  minus
 select INSTITUTION, AID_YEAR, ACAD_CAREER, DISBURSEMENT_PLAN, SPLIT_CODE
   from SYSADM.PS_DISB_SPLIT_CD@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_DISB_SPLIT_CD') = 'Y'
   ) S
