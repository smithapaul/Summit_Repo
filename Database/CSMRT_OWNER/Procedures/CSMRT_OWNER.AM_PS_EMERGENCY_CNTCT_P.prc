DROP PROCEDURE CSMRT_OWNER.AM_PS_EMERGENCY_CNTCT_P
/

--
-- AM_PS_EMERGENCY_CNTCT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_EMERGENCY_CNTCT_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_EMERGENCY_CNTCT';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_EMERGENCY_CNTCT@AMSOURCE S)
 where TABLE_NAME = 'PS_EMERGENCY_CNTCT'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_EMERGENCY_CNTCT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_EMERGENCY_CNTCT';
    merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_EMERGENCY_CNTCT T
    using (select /*+ full(S) */
...
from SYSADM.PS_EMERGENCY_CNTCT@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EMERGENCY_CNTCT') 
  and length(EMPLID) = 8
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_EMERGENCY_CNTCT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_EMERGENCY_CNTCT';
update AMSTG_OWNER.PS_EMERGENCY_CNTCT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, CONTACT_NAME
   from AMSTG_OWNER.PS_EMERGENCY_CNTCT T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EMERGENCY_CNTCT') = 'Y'
  minus
 select nvl(trim(EMPLID),'-') EMPLID, nvl(trim(CONTACT_NAME),'-') CONTACT_NAME
   from SYSADM.PS_EMERGENCY_CNTCT@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EMERGENCY_CNTCT') = 'Y'
   ) S
