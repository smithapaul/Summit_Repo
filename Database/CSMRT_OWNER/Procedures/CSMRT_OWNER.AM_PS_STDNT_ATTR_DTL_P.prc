DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_ATTR_DTL_P
/

--
-- AM_PS_STDNT_ATTR_DTL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_ATTR_DTL_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_STDNT_ATTR_DTL@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_STDNT_ATTR_DTL';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_STDNT_ATTR_DTL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_STDNT_ATTR_DTL';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_ATTR_DTL T
using (select /*+ full(S) */
...
from SYSADM.PS_STDNT_ATTR_DTL@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ATTR_DTL') 
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_ATTR_DTL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_ATTR_DTL';

update AMSTG_OWNER.PS_STDNT_ATTR_DTL T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, EFFDT, EFFSEQ, STDNT_ATTR, STDNT_ATTR_VALUE
   from AMSTG_OWNER.PS_STDNT_ATTR_DTL T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ATTR_DTL') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, EFFDT, EFFSEQ, STDNT_ATTR, STDNT_ATTR_VALUE
   from SYSADM.PS_STDNT_ATTR_DTL@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ATTR_DTL') = 'Y' 
   ) S
