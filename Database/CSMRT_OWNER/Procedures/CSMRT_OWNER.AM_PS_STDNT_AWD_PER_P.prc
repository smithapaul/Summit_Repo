DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_AWD_PER_P
/

--
-- AM_PS_STDNT_AWD_PER_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_STDNT_AWD_PER_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STDNT_AWD_PER';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STDNT_AWD_PER@AMSOURCE S)
 where TABLE_NAME = 'PS_STDNT_AWD_PER'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_STDNT_AWD_PER';
...
insert /*+ append */  into AMSTG_OWNER.PS_T_STDNT_AWD_PER
select /*+ full(S) */
...
to_number(ORA_ROWSCN) SRC_SCN
  from SYSADM.PS_STDNT_AWD_PER@AMSOURCE S
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_STDNT_AWD_PER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_STDNT_AWD_PER';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_AWD_PER T
using (select /*+ full(S) */
nvl(trim(EMPLID),'-') EMPLID,
nvl(trim(INSTITUTION),'-') INSTITUTION,
nvl(trim(AID_YEAR),'-') AID_YEAR,
...
COMMENTS COMMENTS
from AMSTG_OWNER.PS_T_STDNT_AWD_PER S
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_AWD_PER') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_AWD_PER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_AWD_PER';
update AMSTG_OWNER.PS_STDNT_AWD_PER T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, AWARD_PERIOD
   from AMSTG_OWNER.PS_STDNT_AWD_PER T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_AWD_PER') = 'Y'
  minus
 select nvl(trim(EMPLID),'-'), nvl(trim(INSTITUTION),'-'), nvl(trim(AID_YEAR),'-'), nvl(trim(AWARD_PERIOD),'-')
   from SYSADM.PS_STDNT_AWD_PER@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_AWD_PER') = 'Y'
   ) S
