DROP PROCEDURE CSMRT_OWNER.UM_F_ADM_APPL_STAT_P
/

--
-- UM_F_ADM_APPL_STAT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."UM_F_ADM_APPL_STAT_P" AUTHID CURRENT_USER IS
...
--insert /*+ append enable_parallel_dml parallel(8) */ into UM_F_ADM_APPL_STAT
insert /*+ append */ into CSMRT_OWNER.UM_F_ADM_APPL_STAT
with
SRVC as (select /*+ PARALLEL(16) INLINE */
 ...
 select /*+ inline parallel(8) */  
       ...
       ADM_APPL_NBR, 
       SRC_SYS_ID,
       max(decode(RECRT_CTGRY_ID,'ABTS','Y','N')) RECRUIT_ABTS_FLAG, 
       max(decode(RECRT_CTGRY_ID,'BSMS','Y','N')) RECRUIT_BSMS_FLAG 
  from CSMRT_OWNER.PS_R_ADM_RECRTR
where RECRT_CTGRY_ID in ('ABTS','BSMS')
group by 
       INSTITUTION_CD, 
       APPLCNT_SID,
       ACAD_CAR_CD, 
...
NVL(RECRUIT_ABTS_FLAG,'N') as RECRUIT_ABTS_FLAG ,
NVL(RECRUIT_BSMS_FLAG,'N') as  RECRUIT_BSMS_FLAG
...
Left outer join RECRUIT
 on F.APPLCNT_SID = RECRUIT.APPLCNT_SID 
 and F.INSTITUTION_CD = RECRUIT.INSTITUTION_CD
 and F.ACAD_CAR_CD = RECRUIT.ACAD_CAR_CD
 and F.ADM_APPL_NBR = RECRUIT.ADM_APPL_NBR
 and F.SRC_SYS_ID = RECRUIT.SRC_SYS_ID
...
MAX(case when F.PROG_ACN_RSN_CD = 'ABTS' then 'Y'  ELSE 'N' end)
    OVER (PARTITION BY F.APPLCNT_SID, F.INSTITUTION_SID, F.ACAD_CAR_SID, F.STU_CAR_NBR, F.ADM_APPL_NBR, F.SRC_SYS_ID
              ORDER BY F.APPLCNT_SID, F.INSTITUTION_SID, F.ACAD_CAR_SID, F.STU_CAR_NBR, F.ADM_APPL_NBR, F.SRC_SYS_ID
          ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) RSN_ABTS_FLAG,                 
MAX(case when F.PROG_ACN_RSN_CD = 'BSMS' then 'Y' ELSE 'N' end)
    OVER (PARTITION BY F.APPLCNT_SID, F.INSTITUTION_SID, F.ACAD_CAR_SID, F.STU_CAR_NBR, F.ADM_APPL_NBR, F.SRC_SYS_ID
              ORDER BY F.APPLCNT_SID, F.INSTITUTION_SID, F.ACAD_CAR_SID, F.STU_CAR_NBR, F.ADM_APPL_NBR, F.SRC_SYS_ID
          ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) RSN_BSMS_FLAG,           
F.APPL_DT_SID,
F.APPL_CNTR_SID,
F.APPL_CREATE_DT_SID,
F.APPL_CREATION_BY,
...
case when (EARLY_CNT >  NO_APPL_CNT
 ...
 case when (RECRUIT_ABTS_FLAG = 'Y' or RSN_ABTS_FLAG = 'Y' )
     then 'Y' else 'N'
 end ABTS_FLAG,
 
  case when (RECRUIT_BSMS_FLAG = 'Y' or RSN_BSMS_FLAG = 'Y' )
     then 'Y' else 'N'
 end BSMS_FLAG,
...
insert into CSMRT_OWNER.UM_F_ADM_APPL_STAT (
...
ENROLL_CNT, ENROLL_FIRST_CNT, APPL_COUNT_ORDER, APPL_TERM_ORDER, MAX_TERM_FLG, FA_APPLIED_FLAG,ABTS_FLAG,BSMS_FLAG)                                              -- 5
 Values
