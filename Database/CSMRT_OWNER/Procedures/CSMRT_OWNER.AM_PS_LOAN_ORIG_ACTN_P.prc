DROP PROCEDURE CSMRT_OWNER.AM_PS_LOAN_ORIG_ACTN_P
/

--
-- AM_PS_LOAN_ORIG_ACTN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_LOAN_ORIG_ACTN_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_LOAN_ORIG_ACTN';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_LOAN_ORIG_ACTN@AMSOURCE S)
 where TABLE_NAME = 'PS_LOAN_ORIG_ACTN'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_LOAN_ORIG_ACTN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_LOAN_ORIG_ACTN';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_LOAN_ORIG_ACTN T
using (select /*+ full(S) */
...
nvl(trim(SFA_CR_RECIP_ID),'-') SFA_CR_RECIP_ID
  from SYSADM.PS_LOAN_ORIG_ACTN@AMSOURCE S
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOAN_ORIG_ACTN')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_LOAN_ORIG_ACTN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_LOAN_ORIG_ACTN';
update AMSTG_OWNER.PS_LOAN_ORIG_ACTN T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, ACAD_CAREER, LOAN_TYPE, LN_APPL_SEQ, ITEM_TYPE, LN_ORIG_ACTN_SEQ
   from AMSTG_OWNER.PS_LOAN_ORIG_ACTN T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOAN_ORIG_ACTN') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, ACAD_CAREER, LOAN_TYPE, LN_APPL_SEQ, ITEM_TYPE, LN_ORIG_ACTN_SEQ
   from SYSADM.PS_LOAN_ORIG_ACTN@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOAN_ORIG_ACTN') = 'Y'
   ) S
