DROP PROCEDURE CSMRT_OWNER.AM_PS_EXT_ACAD_SUM_P
/

--
-- AM_PS_EXT_ACAD_SUM_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_EXT_ACAD_SUM_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_EXT_ACAD_SUM';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_EXT_ACAD_SUM@AMSOURCE S)
 where TABLE_NAME = 'PS_EXT_ACAD_SUM'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_EXT_ACAD_SUM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_EXT_ACAD_SUM';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_EXT_ACAD_SUM T 
using (select /*+ full(S) */
...
--	  nvl(MS_CONVERT_GPA,0) MS_CONVERT_GPA                    -- SMT-8300
    '-' UM_GPA_EXCLUDE,     -- Jan 2022  
    0 UM_EXT_ORG_CR, 
    0 UM_EXT_ORG_QP, 
    0 UM_EXT_ORG_GPA, 
    0 UM_EXT_ORG_CNV_CR, 
    0 UM_EXT_ORG_CNV_GPA, 
    0 UM_EXT_ORG_CNV_QP, 
    '-' UM_GPA_OVERRIDE, 
    '-' UM_1_OVR_HSGPA, 
    0 UM_CONVERT_GPA,
	0 UM_EXT_OR_MTSC_GPA, 
	0 MS_CONVERT_GPA      
from SYSADM.PS_EXT_ACAD_SUM@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_ACAD_SUM') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_EXT_ACAD_SUM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_EXT_ACAD_SUM';
update AMSTG_OWNER.PS_EXT_ACAD_SUM T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, EXT_ORG_ID, EXT_CAREER, EXT_DATA_NBR, EXT_SUMM_TYPE, INSTITUTION
   from AMSTG_OWNER.PS_EXT_ACAD_SUM T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_ACAD_SUM') = 'Y'
  minus
 select EMPLID, EXT_ORG_ID, EXT_CAREER, EXT_DATA_NBR, EXT_SUMM_TYPE, INSTITUTION
   from SYSADM.PS_EXT_ACAD_SUM@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_ACAD_SUM') = 'Y' 
   ) S
