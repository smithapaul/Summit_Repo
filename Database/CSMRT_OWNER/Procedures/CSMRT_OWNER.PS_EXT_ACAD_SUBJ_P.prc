DROP PROCEDURE CSMRT_OWNER.PS_EXT_ACAD_SUBJ_P
/

--
-- PS_EXT_ACAD_SUBJ_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_EXT_ACAD_SUBJ_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_EXT_ACAD_SUBJ';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_EXT_ACAD_SUBJ@SASOURCE S)
 where TABLE_NAME = 'PS_EXT_ACAD_SUBJ'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_EXT_ACAD_SUBJ';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_EXT_ACAD_SUBJ';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_EXT_ACAD_SUBJ T
using (select /*+ full(S) */
...
from SYSADM.PS_EXT_ACAD_SUBJ@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_ACAD_SUBJ') 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_EXT_ACAD_SUBJ';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_EXT_ACAD_SUBJ';
update CSSTG_OWNER.PS_EXT_ACAD_SUBJ T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, EXT_ORG_ID, EXT_CAREER, EXT_DATA_NBR, EXT_SUBJECT_AREA, COURSE_LEVEL
   from CSSTG_OWNER.PS_EXT_ACAD_SUBJ T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_ACAD_SUBJ') = 'Y'
  minus
 select EMPLID, EXT_ORG_ID, EXT_CAREER, EXT_DATA_NBR, EXT_SUBJECT_AREA, COURSE_LEVEL
   from SYSADM.PS_EXT_ACAD_SUBJ@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_ACAD_SUBJ') = 'Y' 
   ) S
