DROP PROCEDURE CSMRT_OWNER.PS_CAMPUS_EVENT_P
/

--
-- PS_CAMPUS_EVENT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_CAMPUS_EVENT_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_CAMPUS_EVENT';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_CAMPUS_EVENT@SASOURCE S)
 where TABLE_NAME = 'PS_CAMPUS_EVENT'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_CAMPUS_EVENT';
...
INSERT /*+ append */
  INTO CSSTG_OWNER.PS_T_CAMPUS_EVENT
SELECT /*+ full(S) */
...
from SYSADM.PS_CAMPUS_EVENT@SASOURCE S;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_CAMPUS_EVENT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_CAMPUS_EVENT';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_CAMPUS_EVENT T 
using (select /*+ full(S) */
    nvl(trim(CAMPUS_EVENT_NBR),'-') CAMPUS_EVENT_NBR, 
    nvl(trim(INSTITUTION),'-') INSTITUTION, 
    nvl(trim(DESCR),'-') DESCR, 
    ...
    nvl(ATTENDEE_NBR_LST,0) ATTENDEE_NBR_LST, 
    to_char(substr(trim(COMMENTS), 1, 4000)) COMMENTS
from CSSTG_OWNER.PS_T_CAMPUS_EVENT S
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CAMPUS_EVENT') ) S 
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_CAMPUS_EVENT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_CAMPUS_EVENT';
update CSSTG_OWNER.PS_CAMPUS_EVENT T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select CAMPUS_EVENT_NBR
   from CSSTG_OWNER.PS_CAMPUS_EVENT T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CAMPUS_EVENT') = 'Y'
  minus
 select CAMPUS_EVENT_NBR
   from SYSADM.PS_CAMPUS_EVENT@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CAMPUS_EVENT') = 'Y'
   ) S
