DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_AWRD_DISB_P
/

--
-- AM_PS_STDNT_AWRD_DISB_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_AWRD_DISB_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STDNT_AWRD_DISB';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STDNT_AWRD_DISB@AMSOURCE S)
 where TABLE_NAME = 'PS_STDNT_AWRD_DISB'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_STDNT_AWRD_DISB';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_STDNT_AWRD_DISB';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_AWRD_DISB T
using (select /*+ full(S) */
    ...
    nvl(trim(STRM),'-') STRM
  from SYSADM.PS_STDNT_AWRD_DISB@AMSOURCE S 
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_AWRD_DISB')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_AWRD_DISB';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_AWRD_DISB';
update AMSTG_OWNER.PS_STDNT_AWRD_DISB T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, ITEM_TYPE, ACAD_CAREER, DISBURSEMENT_ID
   from AMSTG_OWNER.PS_STDNT_AWRD_DISB T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_AWRD_DISB') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, ITEM_TYPE, ACAD_CAREER, DISBURSEMENT_ID
   from SYSADM.PS_STDNT_AWRD_DISB@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_AWRD_DISB') = 'Y'
   ) S
