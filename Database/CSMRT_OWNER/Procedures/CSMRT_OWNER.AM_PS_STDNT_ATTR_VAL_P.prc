DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_ATTR_VAL_P
/

--
-- AM_PS_STDNT_ATTR_VAL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_ATTR_VAL_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_STDNT_ATTR_VAL@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_STDNT_ATTR_VAL';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_STDNT_ATTR_VAL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_STDNT_ATTR_VAL';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_ATTR_VAL T
using (select /*+ full(S) */
...
from SYSADM.PS_STDNT_ATTR_VAL@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ATTR_VAL') ) S
   on (
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_ATTR_VAL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_ATTR_VAL';

update AMSTG_OWNER.PS_STDNT_ATTR_VAL T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select INSTITUTION, STDNT_ATTR, EFFDT, STDNT_ATTR_VALUE
   from AMSTG_OWNER.PS_STDNT_ATTR_VAL T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ATTR_VAL') = 'Y'
  minus
 select INSTITUTION, STDNT_ATTR, EFFDT, STDNT_ATTR_VALUE
   from SYSADM.PS_STDNT_ATTR_VAL@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_ATTR_VAL') = 'Y' 
   ) S
