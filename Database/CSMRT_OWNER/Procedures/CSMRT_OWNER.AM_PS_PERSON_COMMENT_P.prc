DROP PROCEDURE CSMRT_OWNER.AM_PS_PERSON_COMMENT_P
/

--
-- AM_PS_PERSON_COMMENT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_PERSON_COMMENT_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_PERSON_COMMENT';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_PERSON_COMMENT@AMSOURCE S)
 where TABLE_NAME = 'PS_PERSON_COMMENT'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_PERSON_COMMENT';
...
insert /*+ append */  into AMSTG_OWNER.PS_T_PERSON_COMMENT
select /*+ full(S) */
...
from SYSADM.PS_PERSON_COMMENT@AMSOURCE S
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_PERSON_COMMENT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_PERSON_COMMENT';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_PERSON_COMMENT T 
    using (select /*+ full(S) */
    COMMON_ID, 
    SEQ_3C, 
    SA_ID_TYPE, 
    ...
    COMMENTS,
    to_number(ORA_ROWSCN) SRC_SCN
from AMSTG_OWNER.PS_T_PERSON_COMMENT S
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_COMMENT') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_PERSON_COMMENT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_PERSON_COMMENT';
update AMSTG_OWNER.PS_PERSON_COMMENT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select COMMON_ID, SEQ_3C
   from AMSTG_OWNER.PS_PERSON_COMMENT T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_COMMENT') = 'Y'
  minus
 select COMMON_ID, SEQ_3C
   from SYSADM.PS_PERSON_COMMENT@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_COMMENT') = 'Y'
   ) S
