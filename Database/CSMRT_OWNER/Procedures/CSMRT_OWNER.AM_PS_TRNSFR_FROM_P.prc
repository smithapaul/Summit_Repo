DROP PROCEDURE CSMRT_OWNER.AM_PS_TRNSFR_FROM_P
/

--
-- AM_PS_TRNSFR_FROM_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_TRNSFR_FROM_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_TRNSFR_FROM';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_TRNSFR_FROM@AMSOURCE S)
 where TABLE_NAME = 'PS_TRNSFR_FROM'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_TRNSFR_FROM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_TRNSFR_FROM';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_TRNSFR_FROM T
using (select /*+ full(S) */
...
from SYSADM.PS_TRNSFR_FROM@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNSFR_FROM') ) S
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_TRNSFR_FROM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_TRNSFR_FROM';
update AMSTG_OWNER.PS_TRNSFR_FROM T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select INSTITUTION, TRNSFR_SRC_ID, COMP_SUBJECT_AREA, EFFDT, TRNSFR_EQVLNCY_CMP, TRNSFR_CMP_SEQ
   from AMSTG_OWNER.PS_TRNSFR_FROM T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNSFR_FROM') = 'Y'
  minus
 select INSTITUTION, TRNSFR_SRC_ID, COMP_SUBJECT_AREA, EFFDT, TRNSFR_EQVLNCY_CMP, TRNSFR_CMP_SEQ
   from SYSADM.PS_TRNSFR_FROM@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNSFR_FROM') = 'Y' 
   ) S
