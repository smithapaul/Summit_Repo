DROP PROCEDURE CSMRT_OWNER.AM_PS_PERSON_CHK_ITEM_P
/

--
-- AM_PS_PERSON_CHK_ITEM_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_PERSON_CHK_ITEM_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_PERSON_CHK_ITEM';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_PERSON_CHK_ITEM@AMSOURCE S)
 where TABLE_NAME = 'PS_PERSON_CHK_ITEM'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_PERSON_CHK_ITEM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_PERSON_CHK_ITEM';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_PERSON_CHK_ITEM T
using (select /*+ full(S) */
...
--    to_date(to_char(case when DUE_DT < '01-JAN-1800' then NULL else DUE_DT end,'MM/DD/YYYY HH24:MI:SS'),'MM/DD/YYYY HH24:MI:SS') DUE_DT,
    to_date(to_char(case when DUE_DT < '01-JAN-1800' then to_date('01-JAN-1800') else DUE_DT end,'MM/DD/YYYY HH24:MI:SS'),'MM/DD/YYYY HH24:MI:SS') DUE_DT,   -- Jan 2022 
...
from SYSADM.PS_PERSON_CHK_ITEM@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_CHK_ITEM')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_PERSON_CHK_ITEM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_PERSON_CHK_ITEM';
update AMSTG_OWNER.PS_PERSON_CHK_ITEM T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select COMMON_ID, SEQ_3C, CHECKLIST_SEQ
   from AMSTG_OWNER.PS_PERSON_CHK_ITEM T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_CHK_ITEM') = 'Y'
  minus
 select COMMON_ID, SEQ_3C, CHECKLIST_SEQ
   from SYSADM.PS_PERSON_CHK_ITEM@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_CHK_ITEM') = 'Y'
   ) S
