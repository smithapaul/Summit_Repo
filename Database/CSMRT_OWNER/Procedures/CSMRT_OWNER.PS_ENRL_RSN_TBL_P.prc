DROP PROCEDURE CSMRT_OWNER.PS_ENRL_RSN_TBL_P
/

--
-- PS_ENRL_RSN_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_ENRL_RSN_TBL_P"
   AUTHID CURRENT_USER
IS
   /*
...
BEGIN
   ...
   UPDATE CSSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_ENRL_RSN_TBL@SASOURCE S)
    WHERE TABLE_NAME = 'PS_ENRL_RSN_TBL';
   ...
   strMessage01 := 'Merging data into CSSTG_OWNER.PS_ENRL_RSN_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into CSSTG_OWNER.PS_ENRL_RSN_TBL';

   MERGE /*+ use_hash(S,T) */
        INTO  CSSTG_OWNER.PS_ENRL_RSN_TBL T
        USING (SELECT /*+ full(S) */
   ...
   strMessage01 := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_ENRL_RSN_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on CSSTG_OWNER.PS_ENRL_RSN_TBL';

   UPDATE CSSTG_OWNER.PS_ENRL_RSN_TBL T
          ...
          AND EXISTS
                 (SELECT 1
                    FROM (SELECT SETID,
                                 ...
                                 ENRL_ACTION_REASON,
                                 EFFDT
                            FROM CSSTG_OWNER.PS_ENRL_RSN_TBL T2
                          ...
                          SELECT SETID,
                                 ...
                                 ENRL_ACTION_REASON,
                                 EFFDT
                            FROM SYSADM.PS_ENRL_RSN_TBL@SASOURCE
                           WHERE (SELECT DELETE_FLG
                                    FROM CSSTG_OWNER.UM_STAGE_JOBS
                                   WHERE TABLE_NAME = 'PS_ENRL_RSN_TBL') =
                                    'Y') S
