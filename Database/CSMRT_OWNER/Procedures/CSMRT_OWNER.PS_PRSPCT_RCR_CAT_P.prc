DROP PROCEDURE CSMRT_OWNER.PS_PRSPCT_RCR_CAT_P
/

--
-- PS_PRSPCT_RCR_CAT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_PRSPCT_RCR_CAT_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_PRSPCT_RCR_CAT';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_PRSPCT_RCR_CAT@SASOURCE S)
 where TABLE_NAME = 'PS_PRSPCT_RCR_CAT'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_PRSPCT_RCR_CAT';
...
insert /*+ append */  into CSSTG_OWNER.PS_T_PRSPCT_RCR_CAT
select /*+ full(S) */
...
from SYSADM.PS_PRSPCT_RCR_CAT@SASOURCE S
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_PRSPCT_RCR_CAT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_PRSPCT_RCR_CAT';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_PRSPCT_RCR_CAT T 
using (select /*+ full(S) */
    nvl(trim(EMPLID),'-') EMPLID, 
    nvl(trim(ACAD_CAREER),'-') ACAD_CAREER, 
    nvl(trim(INSTITUTION),'-') INSTITUTION, 
    ...
    nvl(trim(PROMPT_RECRUITER),'-') PROMPT_RECRUITER, 
    DESCRLONG
from CSSTG_OWNER.PS_T_PRSPCT_RCR_CAT S
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PRSPCT_RCR_CAT') ) S 
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_PRSPCT_RCR_CAT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_PRSPCT_RCR_CAT';
update CSSTG_OWNER.PS_PRSPCT_RCR_CAT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, INSTITUTION, RECRUITMENT_CAT
   from CSSTG_OWNER.PS_PRSPCT_RCR_CAT T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PRSPCT_RCR_CAT') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, INSTITUTION, RECRUITMENT_CAT
   from SYSADM.PS_PRSPCT_RCR_CAT@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PRSPCT_RCR_CAT') = 'Y'
   ) S
