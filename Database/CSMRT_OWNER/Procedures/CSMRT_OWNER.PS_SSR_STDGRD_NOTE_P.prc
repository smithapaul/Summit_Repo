DROP PROCEDURE CSMRT_OWNER.PS_SSR_STDGRD_NOTE_P
/

--
-- PS_SSR_STDGRD_NOTE_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_SSR_STDGRD_NOTE_P"
   AUTHID CURRENT_USER
IS
   /*
   ...
   */
   ...
   ------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_SSR_STDGRD_NOTE';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_SSR_STDGRD_NOTE@SASOURCE S)
 where TABLE_NAME = 'PS_SSR_STDGRD_NOTE'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_SSR_STDGRD_NOTE';
...
INSERT /*+ append */
      INTO  CSSTG_OWNER.PS_T_SSR_STDGRD_NOTE
SELECT /*+ full(S) */
        EMPLID, 
		INSTITUTION, 
		ACAD_CAREER, 
   ...
   FROM SYSADM.PS_SSR_STDGRD_NOTE@SASOURCE S
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_SSR_STDGRD_NOTE';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_SSR_STDGRD_NOTE';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_SSR_STDGRD_NOTE T 
using (select /*+ full(S) */ 
...
from CSSTG_OWNER.PS_T_SSR_STDGRD_NOTE S 
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SSR_STDGRD_NOTE') ) S 
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_SSR_STDGRD_NOTE';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_SSR_STDGRD_NOTE';
update CSSTG_OWNER.PS_SSR_STDGRD_NOTE T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select nvl(trim(EMPLID),'-') EMPLID, 
	    ...
	    nvl(trim(DEGREE),'-') DEGREE, 
	    nvl(SEQNUM,0) SEQNUM
   from CSSTG_OWNER.PS_SSR_STDGRD_NOTE T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SSR_STDGRD_NOTE') = 'Y'
  minus
 select EMPLID, INSTITUTION, ACAD_CAREER, STDNT_CAR_NBR, ACAD_PROG, nvl(trim(EXP_GRAD_TERM),'-') EXP_GRAD_TERM, DEGREE, SEQNUM
   from SYSADM.PS_SSR_STDGRD_NOTE@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SSR_STDGRD_NOTE') = 'Y'
   ) S
