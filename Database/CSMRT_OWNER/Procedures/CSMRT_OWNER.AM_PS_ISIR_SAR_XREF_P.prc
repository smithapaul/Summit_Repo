DROP PROCEDURE CSMRT_OWNER.AM_PS_ISIR_SAR_XREF_P
/

--
-- AM_PS_ISIR_SAR_XREF_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ISIR_SAR_XREF_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ISIR_SAR_XREF';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ISIR_SAR_XREF@AMSOURCE S)
 where TABLE_NAME = 'PS_ISIR_SAR_XREF'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ISIR_SAR_XREF';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ISIR_SAR_XREF';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ISIR_SAR_XREF T
using (select /*+ full(S) */
  ...
  from SYSADM.PS_ISIR_SAR_XREF@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_SAR_XREF') ) S
    on (
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ISIR_SAR_XREF';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ISIR_SAR_XREF';
update AMSTG_OWNER.PS_ISIR_SAR_XREF T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select AID_YEAR, RECNAME, FIELDNAME
   from AMSTG_OWNER.PS_ISIR_SAR_XREF T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_SAR_XREF') = 'Y'
  minus
 select nvl(trim(AID_YEAR),'-'), nvl(trim(RECNAME),'-'), nvl(trim(FIELDNAME),'-')
   from SYSADM.PS_ISIR_SAR_XREF@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_SAR_XREF') = 'Y'
   ) S
