...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_TRNSFR_COMP';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_TRNSFR_COMP@SASOURCE S)
 where TABLE_NAME = 'PS_TRNSFR_COMP'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_TRNSFR_COMP';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_TRNSFR_COMP';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_TRNSFR_COMP T
using (select /*+ full(S) */
...
from SYSADM.PS_TRNSFR_COMP@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNSFR_COMP') ) S
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_TRNSFR_COMP';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_TRNSFR_COMP';
update CSSTG_OWNER.PS_TRNSFR_COMP T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select INSTITUTION, TRNSFR_SRC_ID, COMP_SUBJECT_AREA, EFFDT, TRNSFR_EQVLNCY_CMP
   from CSSTG_OWNER.PS_TRNSFR_COMP T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNSFR_COMP') = 'Y'
  minus
 select INSTITUTION, TRNSFR_SRC_ID, COMP_SUBJECT_AREA, EFFDT, TRNSFR_EQVLNCY_CMP
   from SYSADM.PS_TRNSFR_COMP@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNSFR_COMP') = 'Y' 
   ) S
