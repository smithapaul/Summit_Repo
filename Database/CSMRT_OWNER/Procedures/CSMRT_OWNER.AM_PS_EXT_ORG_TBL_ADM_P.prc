DROP PROCEDURE CSMRT_OWNER.AM_PS_EXT_ORG_TBL_ADM_P
/

--
-- AM_PS_EXT_ORG_TBL_ADM_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_EXT_ORG_TBL_ADM_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_EXT_ORG_TBL_ADM';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_EXT_ORG_TBL_ADM@AMSOURCE S)
 where TABLE_NAME = 'PS_EXT_ORG_TBL_ADM'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_EXT_ORG_TBL_ADM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_EXT_ORG_TBL_ADM';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_EXT_ORG_TBL_ADM T
using (select /*+ full(S) */
...
from SYSADM.PS_EXT_ORG_TBL_ADM@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_ORG_TBL_ADM') ) S
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_EXT_ORG_TBL_ADM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_EXT_ORG_TBL_ADM';
update AMSTG_OWNER.PS_EXT_ORG_TBL_ADM T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EXT_ORG_ID, EFFDT
   from AMSTG_OWNER.PS_EXT_ORG_TBL_ADM T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_ORG_TBL_ADM') = 'Y'
  minus
 select EXT_ORG_ID, EFFDT
   from SYSADM.PS_EXT_ORG_TBL_ADM@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_ORG_TBL_ADM') = 'Y' 
   ) S
