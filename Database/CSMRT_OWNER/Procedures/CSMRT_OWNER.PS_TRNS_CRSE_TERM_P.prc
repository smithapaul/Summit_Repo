DROP PROCEDURE CSMRT_OWNER.PS_TRNS_CRSE_TERM_P
/

--
-- PS_TRNS_CRSE_TERM_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_TRNS_CRSE_TERM_P" AUTHID CURRENT_USER IS


------------------------------------------------------------------------
-- George Adams
--
-- Loads stage table PS_TRNS_CRSE_TERM from PeopleSoft table PS_TRNS_CRSE_TERM.
--
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_TRNS_CRSE_TERM';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Reading',
       START_DT = sysdate,
       END_DT = NULL
 where TABLE_NAME = 'PS_TRNS_CRSE_TERM'
;
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_TRNS_CRSE_TERM@SASOURCE S)
 where TABLE_NAME = 'PS_TRNS_CRSE_TERM'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_TRNS_CRSE_TERM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_TRNS_CRSE_TERM';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_TRNS_CRSE_TERM T 
using (select /*+ full(S) */
    ...
    nvl(SSR_FAWI_TKN,0) SSR_FAWI_TKN, 
...
from SYSADM.PS_TRNS_CRSE_TERM@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_CRSE_TERM')
...
where 
    ...
    T.SSR_FAWI_TKN <> S.SSR_FAWI_TKN or 
...
strMessage01    := '# of PS_TRNS_CRSE_TERM rows merged: ' || TO_CHAR(intRowCount,'999,999,999,999');
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand := 'SMT_PROCESS_LOG.PROCESS_DETAIL';
COMMON_OWNER.SMT_PROCESS_LOG.PROCESS_DETAIL
        (
                i_TargetTableName   => 'PS_TRNS_CRSE_TERM',
                i_Action            => 'MERGE',
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_TRNS_CRSE_TERM';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_TRNS_CRSE_TERM';
update CSSTG_OWNER.PS_TRNS_CRSE_TERM T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, INSTITUTION, MODEL_NBR, ARTICULATION_TERM
   from CSSTG_OWNER.PS_TRNS_CRSE_TERM T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_CRSE_TERM') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, INSTITUTION, MODEL_NBR, ARTICULATION_TERM
   from SYSADM.PS_TRNS_CRSE_TERM@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_CRSE_TERM') = 'Y' 
   ) S
...
strMessage01    := '# of PS_TRNS_CRSE_TERM rows updated: ' || TO_CHAR(intRowCount,'999,999,999,999');
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);
