DROP PROCEDURE CSMRT_OWNER.AM_PS_ITEM_SF_P
/

--
-- AM_PS_ITEM_SF_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_ITEM_SF_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ITEM_SF';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ITEM_SF@AMSOURCE S)
 where TABLE_NAME = 'PS_ITEM_SF'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ITEM_SF';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ITEM_SF';
merge /*+ use_hash(S,T) parallel(8) enable_parallel_dml */ into AMSTG_OWNER.PS_ITEM_SF T
using (select /*+ full(S) */
...
to_date(to_char(SCC_ROW_UPD_DTTM,'MM/DD/YYYY HH24:MI:SS'),'MM/DD/YYYY HH24:MI:SS') SCC_ROW_UPD_DTTM
  from SYSADM.PS_ITEM_SF@AMSOURCE S
-- where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ITEM_SF')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ITEM_SF';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ITEM_SF';
update /*+ parallel(8) enable_parallel_dml */ AMSTG_OWNER.PS_ITEM_SF T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select BUSINESS_UNIT, COMMON_ID, SA_ID_TYPE, ITEM_NBR
   from AMSTG_OWNER.PS_ITEM_SF T2
--  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ITEM_SF') = 'Y'
  minus
 select BUSINESS_UNIT, COMMON_ID, SA_ID_TYPE, ITEM_NBR
   from SYSADM.PS_ITEM_SF@AMSOURCE S2
--  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ITEM_SF') = 'Y'
   ) S
