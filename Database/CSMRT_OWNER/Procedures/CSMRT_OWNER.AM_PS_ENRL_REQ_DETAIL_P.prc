DROP PROCEDURE CSMRT_OWNER.AM_PS_ENRL_REQ_DETAIL_P
/

--
-- AM_PS_ENRL_REQ_DETAIL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ENRL_REQ_DETAIL_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ENRL_REQ_DETAIL';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ENRL_REQ_DETAIL@AMSOURCE S)
 where TABLE_NAME = 'PS_ENRL_REQ_DETAIL'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ENRL_REQ_DETAIL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ENRL_REQ_DETAIL';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ENRL_REQ_DETAIL T
using (select /*+ full(S) */
...
from SYSADM.PS_ENRL_REQ_DETAIL@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ENRL_REQ_DETAIL') 
  and STRM >= '2010'
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ENRL_REQ_DETAIL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ENRL_REQ_DETAIL';
update AMSTG_OWNER.PS_ENRL_REQ_DETAIL T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select ENRL_REQUEST_ID, ENRL_REQ_DETL_SEQ
   from AMSTG_OWNER.PS_ENRL_REQ_DETAIL T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ENRL_REQ_DETAIL') = 'Y'
  minus
 select ENRL_REQUEST_ID, ENRL_REQ_DETL_SEQ
   from SYSADM.PS_ENRL_REQ_DETAIL@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ENRL_REQ_DETAIL') = 'Y'
   ) S
