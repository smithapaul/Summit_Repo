DROP PROCEDURE CSMRT_OWNER.PS_STND_MTGPAT_TBL_P
/

--
-- PS_STND_MTGPAT_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_STND_MTGPAT_TBL_P"
   AUTHID CURRENT_USER
IS
   /*
...
BEGIN
   ...
   UPDATE CSSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_STND_MTGPAT_TBL@SASOURCE S)
    WHERE TABLE_NAME = 'PS_STND_MTGPAT_TBL';
   ...
   strMessage01 := 'Merging data into CSSTG_OWNER.PS_STND_MTGPAT_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into CSSTG_OWNER.PS_STND_MTGPAT_TBL';

   MERGE /*+ use_hash(S,T) */
        INTO  CSSTG_OWNER.PS_STND_MTGPAT_TBL T
        USING (SELECT /*+ full(S) */
   ...
   strMessage01 := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_STND_MTGPAT_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on CSSTG_OWNER.PS_STND_MTGPAT_TBL';

   UPDATE CSSTG_OWNER.PS_STND_MTGPAT_TBL T
          ...
          AND EXISTS
                 (SELECT 1
                    FROM (SELECT INSTITUTION,
                                 ...
                                 EFFDT,
                                 STND_MTG_PAT
                            FROM CSSTG_OWNER.PS_STND_MTGPAT_TBL T2
                          ...
                          SELECT INSTITUTION,
                                 ...
                                 EFFDT,
                                 STND_MTG_PAT
                            FROM SYSADM.PS_STND_MTGPAT_TBL@SASOURCE
                           WHERE (SELECT DELETE_FLG
                                    FROM CSSTG_OWNER.UM_STAGE_JOBS
                                   WHERE TABLE_NAME = 'PS_STND_MTGPAT_TBL') =
                                    'Y') S
