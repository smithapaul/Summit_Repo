DROP PROCEDURE CSMRT_OWNER.PS_ETHNICITY_DTL_P
/

--
-- PS_ETHNICITY_DTL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_ETHNICITY_DTL_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_ETHNICITY_DTL';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ETHNICITY_DTL@SASOURCE S)
 where TABLE_NAME = 'PS_ETHNICITY_DTL'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_ETHNICITY_DTL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_ETHNICITY_DTL';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_ETHNICITY_DTL T
using (select /*+ full(S) */
...
from SYSADM.PS_ETHNICITY_DTL@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ETHNICITY_DTL') 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_ETHNICITY_DTL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_ETHNICITY_DTL';
update CSSTG_OWNER.PS_ETHNICITY_DTL T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, REG_REGION, ETHNIC_GRP_CD
   from CSSTG_OWNER.PS_ETHNICITY_DTL T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ETHNICITY_DTL') = 'Y'
  minus
 select EMPLID, REG_REGION, ETHNIC_GRP_CD
   from SYSADM.PS_ETHNICITY_DTL@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ETHNICITY_DTL') = 'Y' 
   ) S
