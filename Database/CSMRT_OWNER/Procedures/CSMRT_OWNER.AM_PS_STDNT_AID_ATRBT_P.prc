DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_AID_ATRBT_P
/

--
-- AM_PS_STDNT_AID_ATRBT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_AID_ATRBT_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STDNT_AID_ATRBT';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STDNT_AID_ATRBT@AMSOURCE S)
 where TABLE_NAME = 'PS_STDNT_AID_ATRBT'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_STDNT_AID_ATRBT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_STDNT_AID_ATRBT';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_AID_ATRBT T
using (select /*+ full(S) */
...
from SYSADM.PS_STDNT_AID_ATRBT@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_AID_ATRBT') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_AID_ATRBT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_AID_ATRBT';
update AMSTG_OWNER.PS_STDNT_AID_ATRBT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, TABLE_ID
   from AMSTG_OWNER.PS_STDNT_AID_ATRBT T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_AID_ATRBT') = 'Y'
  minus
 select nvl(trim(EMPLID),'-'), nvl(trim(INSTITUTION),'-'), nvl(trim(AID_YEAR),'-'), nvl(trim(TABLE_ID),'-')
   from SYSADM.PS_STDNT_AID_ATRBT@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_AID_ATRBT') = 'Y'
   ) S
