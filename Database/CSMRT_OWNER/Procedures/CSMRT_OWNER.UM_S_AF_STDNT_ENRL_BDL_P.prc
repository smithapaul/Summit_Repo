DROP PROCEDURE CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL_P
/

--
-- UM_S_AF_STDNT_ENRL_BDL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."UM_S_AF_STDNT_ENRL_BDL_P"
        (
...
-- James Doucette
--
-- Loads table UM_S_AF_STDNT_ENRL_BDL.
--
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'UM_S_AF_STDNT_ENRL_BDL';
...
BEGIN
   ...
   --verifying the job is not running twice on the same day and checking whether UM_M_AF_STDNT_ENRL_AMH table loaded with the current day's file 
    ...
    select  max(effdt_start) STARTDATE, max(effdt_end) ENDDATE  from CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL
    )
    where (STARTDATE = trunc(sysdate) OR ENDDATE = trunc(sysdate));

    select decode(count(*),0,'SOURCEEMPTY','SOURCELOADED') into M_LoadStatus from UM_M_AF_STDNT_ENRL_BDL  where trunc(created_ew_dttm) = trunc(sysdate);
...
strMessage01    := 'Disabling Indexes for table CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);
COMMON_OWNER.SMT_INDEX.ALL_UNUSABLE('CSMRT_OWNER','UM_S_AF_STDNT_ENRL_BDL');

strSqlDynamic   := 'alter table CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL disable constraint PK_UM_S_AF_STDNT_ENRL_BDL';
...
strMessage01    := 'Merging data into CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL';
merge into CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL T
 using (
  with Q1 as (
select /*+ inline parallel(8) */
  ...
  from CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL
 group by INSTITUTION_CD, ACAD_CAR_CD, TERM_CD, PERSON_ID, SRC_SYS_ID),
       Q2 as (
--SELECT max(trunc(START_TIME)) MAX_START_DT
--  FROM COMMON_OWNER.JOB_LOG
-- WHERE OWNER = 'CSMRT_OWNER'
--   and JOB_ACTION = 'CSMRT_OWNER."UM_S_AF_STDNT_ENRL_BDL_P"'
--   and STATUS = 'SUCCEEDED'
--   and JOB_TYPE = 'STORED_PROCEDURE'
...
select /*+ inline parallel(8) */
       ...
       ONLINE_FTE, TOT_FTE, ONLINE_CREDITS, CE_ONLINE_CREDITS, NON_ONLINE_CREDITS, CE_CREDITS, NON_CE_CREDITS, TOT_CREDITS, ENROLL_CNT, ONLINE_CNT, CE_CNT  -- Nov 2020
  from CSMRT_OWNER.UM_M_AF_STDNT_ENRL_BDL S
  left outer join Q1
  ...
  and  nvl(round(T.CE_ONLINE_CREDITS,9),0) = nvl(round(S.CE_ONLINE_CREDITS,9),0)        -- Nov 2020
...
strMessage01    := '# of UM_S_AF_STDNT_ENRL_BDL rows merged: ' || TO_CHAR(intRowCount,'999,999,999,999');
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand := 'SMT_PROCESS_LOG.PROCESS_DETAIL';
COMMON_OWNER.SMT_PROCESS_LOG.PROCESS_DETAIL
        (
                i_TargetTableName   => 'UM_S_AF_STDNT_ENRL_BDL',
                i_Action            => 'MERGE',
                i_RowCount          => intRowCount
        );

strMessage01    := 'Enabling Indexes for table CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlDynamic   := 'alter table CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL enable constraint PK_UM_S_AF_STDNT_ENRL_BDL';
strSqlCommand   := 'SMT_UTILITY.EXECUTE_IMMEDIATE: ' || strSqlDynamic;
COMMON_OWNER.SMT_UTILITY.EXECUTE_IMMEDIATE
                (
                i_SqlStatement          => strSqlDynamic,
...
COMMON_OWNER.SMT_INDEX.ALL_REBUILD('CSMRT_OWNER','UM_S_AF_STDNT_ENRL_BDL');
...
strMessage01    := 'Updating data in CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL to prevent gaps';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL';
update CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL T
 ...
 where T.EFFDT_START < trunc(dtEFFDT)
   and T.EFFDT_END between (select max(EFFDT_END) MAX_EFFDT_END
                              from CSMRT_OWNER.UM_S_AF_STDNT_ENRL_BDL        -- EFFDT_END prior to the preceding merge.
...
strMessage01    := '# of UM_S_AF_STDNT_ENRL_BDL rows updated: ' || TO_CHAR(intRowCount,'999,999,999,999');
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);
