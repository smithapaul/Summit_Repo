DROP PROCEDURE CSMRT_OWNER.PS_ADM_APPL_SBPLAN_P
/

--
-- PS_ADM_APPL_SBPLAN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_ADM_APPL_SBPLAN_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_ADM_APPL_SBPLAN';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ADM_APPL_SBPLAN@SASOURCE S)
 where TABLE_NAME = 'PS_ADM_APPL_SBPLAN'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_ADM_APPL_SBPLAN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_ADM_APPL_SBPLAN';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_ADM_APPL_SBPLAN T
using (select /*+ full(S) */
...
from SYSADM.PS_ADM_APPL_SBPLAN@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_SBPLAN')
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_ADM_APPL_SBPLAN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_ADM_APPL_SBPLAN';
update CSSTG_OWNER.PS_ADM_APPL_SBPLAN T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR, APPL_PROG_NBR, EFFDT, EFFSEQ, ACAD_PLAN, ACAD_SUB_PLAN
   from CSSTG_OWNER.PS_ADM_APPL_SBPLAN T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_SBPLAN') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR, APPL_PROG_NBR, EFFDT, EFFSEQ, ACAD_PLAN, ACAD_SUB_PLAN
   from SYSADM.PS_ADM_APPL_SBPLAN@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_SBPLAN') = 'Y' 
   ) S
