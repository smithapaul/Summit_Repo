DROP PROCEDURE CSMRT_OWNER.AM_PS_ITEM_TYPE_FISCL_P
/

--
-- AM_PS_ITEM_TYPE_FISCL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ITEM_TYPE_FISCL_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_ITEM_TYPE_FISCL@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_ITEM_TYPE_FISCL';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_ITEM_TYPE_FISCL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_ITEM_TYPE_FISCL';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ITEM_TYPE_FISCL T
using (select /*+ full(S) */
...
from SYSADM.PS_ITEM_TYPE_FISCL@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ITEM_TYPE_FISCL') ) S
 on ( 
    T.SETID = S.SETID and 
...
;
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ITEM_TYPE_FISCL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ITEM_TYPE_FISCL';
update AMSTG_OWNER.PS_ITEM_TYPE_FISCL T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select SETID, ITEM_TYPE, AID_YEAR
   from AMSTG_OWNER.PS_ITEM_TYPE_FISCL T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ITEM_TYPE_FISCL') = 'Y'
  minus
 select SETID, ITEM_TYPE, AID_YEAR
   from SYSADM.PS_ITEM_TYPE_FISCL@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ITEM_TYPE_FISCL') = 'Y'
   ) S
