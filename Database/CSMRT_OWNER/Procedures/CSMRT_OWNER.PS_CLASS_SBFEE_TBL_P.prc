DROP PROCEDURE CSMRT_OWNER.PS_CLASS_SBFEE_TBL_P
/

--
-- PS_CLASS_SBFEE_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_CLASS_SBFEE_TBL_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_CLASS_SBFEE_TBL';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_CLASS_SBFEE_TBL@SASOURCE S)
 where TABLE_NAME = 'PS_CLASS_SBFEE_TBL'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_CLASS_SBFEE_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_CLASS_SBFEE_TBL';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_CLASS_SBFEE_TBL T
using (select /*+ full(S) */
    ...
    DYNAMIC_ORG,
    SSF_EXCLUDE_HECS
  from SYSADM.PS_CLASS_SBFEE_TBL@SASOURCE S 
 where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_SBFEE_TBL') 
    )S
 on ( 
    T.SETID = S.SETID and 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_CLASS_SBFEE_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_CLASS_SBFEE_TBL';
update CSSTG_OWNER.PS_CLASS_SBFEE_TBL T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select nvl(trim(SETID),'-') SETID, nvl(trim(CRSE_ID),'-') CRSE_ID, nvl(CRSE_OFFER_NBR,0) CRSE_OFFER_NBR, 
   ...
   from CSSTG_OWNER.PS_CLASS_SBFEE_TBL T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_SBFEE_TBL') = 'Y'
  minus
 select nvl(trim(SETID),'-') SETID, nvl(trim(CRSE_ID),'-') CRSE_ID, nvl(CRSE_OFFER_NBR,0) CRSE_OFFER_NBR, 
        nvl(trim(STRM),'-') STRM, nvl(trim(SESSION_CODE),'-') SESSION_CODE, nvl(trim(CLASS_SECTION),'-') CLASS_SECTION, 
        nvl(trim(SSR_COMPONENT),'-') SSR_COMPONENT, nvl(trim(ACCOUNT_TYPE_SF),'-') ACCOUNT_TYPE_SF, nvl(trim(ITEM_TYPE),'-') ITEM_TYPE
   from SYSADM.PS_CLASS_SBFEE_TBL@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_SBFEE_TBL') = 'Y'
   ) S
