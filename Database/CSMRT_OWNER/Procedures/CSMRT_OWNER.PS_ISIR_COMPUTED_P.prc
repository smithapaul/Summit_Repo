DROP PROCEDURE CSMRT_OWNER.PS_ISIR_COMPUTED_P
/

--
-- PS_ISIR_COMPUTED_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_ISIR_COMPUTED_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_ISIR_COMPUTED';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ISIR_COMPUTED@SASOURCE S)
 where TABLE_NAME = 'PS_ISIR_COMPUTED'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_ISIR_COMPUTED';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_ISIR_COMPUTED';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_ISIR_COMPUTED T
using (select /*+ full(S) */
  ...
  from SYSADM.PS_ISIR_COMPUTED@SASOURCE S 
 where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_COMPUTED')
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_ISIR_COMPUTED';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_ISIR_COMPUTED';
update CSSTG_OWNER.PS_ISIR_COMPUTED T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, EFFDT, EFFSEQ, TABLE_ID
   from CSSTG_OWNER.PS_ISIR_COMPUTED T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_COMPUTED') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, EFFDT, EFFSEQ, TABLE_ID
   from SYSADM.PS_ISIR_COMPUTED@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_COMPUTED') = 'Y'
   ) S
