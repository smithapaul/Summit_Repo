DROP PROCEDURE CSMRT_OWNER.PS_VAR_DATA_AVAK_P
/

--
-- PS_VAR_DATA_AVAK_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_VAR_DATA_AVAK_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_VAR_DATA_AVAK';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_VAR_DATA_AVAK@SASOURCE S)
 where TABLE_NAME = 'PS_VAR_DATA_AVAK'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_VAR_DATA_AVAK';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_VAR_DATA_AVAK';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_VAR_DATA_AVAK T
using (select /*+ full(S) */
...
from SYSADM.PS_VAR_DATA_AVAK@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_VAR_DATA_AVAK')
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_VAR_DATA_AVAK';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_VAR_DATA_AVAK';
update CSSTG_OWNER.PS_VAR_DATA_AVAK T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select COMMON_ID, VAR_DATA_SEQ
   from CSSTG_OWNER.PS_VAR_DATA_AVAK T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_VAR_DATA_AVAK') = 'Y'
  minus
 select COMMON_ID, VAR_DATA_SEQ
   from SYSADM.PS_VAR_DATA_AVAK@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_VAR_DATA_AVAK') = 'Y' 
   ) S
