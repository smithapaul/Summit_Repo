DROP PROCEDURE CSMRT_OWNER.AM_PS_CLST_MAIN_TBL_P
/

--
-- AM_PS_CLST_MAIN_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_CLST_MAIN_TBL_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_CLST_MAIN_TBL';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_CLST_MAIN_TBL@AMSOURCE S)
 where TABLE_NAME = 'PS_CLST_MAIN_TBL'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_CLST_MAIN_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_CLST_MAIN_TBL';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_CLST_MAIN_TBL T
using (select /*+ full(S) */
    ...
    DESCR254A DESCR254A
    from SYSADM.PS_CLST_MAIN_TBL@AMSOURCE S 
    where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLST_MAIN_TBL') ) S 
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_CLST_MAIN_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_CLST_MAIN_TBL';
update AMSTG_OWNER.PS_CLST_MAIN_TBL T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select COURSE_LIST, EFFDT
   from AMSTG_OWNER.PS_CLST_MAIN_TBL T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLST_MAIN_TBL') = 'Y'
  minus
 select COURSE_LIST, EFFDT
   from SYSADM.PS_CLST_MAIN_TBL@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLST_MAIN_TBL') = 'Y'
   ) S
