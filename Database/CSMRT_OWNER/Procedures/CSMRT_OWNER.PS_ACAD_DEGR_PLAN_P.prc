DROP PROCEDURE CSMRT_OWNER.PS_ACAD_DEGR_PLAN_P
/

--
-- PS_ACAD_DEGR_PLAN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_ACAD_DEGR_PLAN_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_ACAD_DEGR_PLAN';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ACAD_DEGR_PLAN@SASOURCE S)
 where TABLE_NAME = 'PS_ACAD_DEGR_PLAN'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_ACAD_DEGR_PLAN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_ACAD_DEGR_PLAN';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_ACAD_DEGR_PLAN T
using (select /*+ full(S) */
...
from SYSADM.PS_ACAD_DEGR_PLAN@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_DEGR_PLAN') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_ACAD_DEGR_PLAN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_ACAD_DEGR_PLAN';
update CSSTG_OWNER.PS_ACAD_DEGR_PLAN T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, STDNT_DEGR, ACAD_PLAN
   from CSSTG_OWNER.PS_ACAD_DEGR_PLAN T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_DEGR_PLAN') = 'Y'
  minus
 select EMPLID, STDNT_DEGR,ACAD_PLAN
   from SYSADM.PS_ACAD_DEGR_PLAN@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_DEGR_PLAN') = 'Y' 
