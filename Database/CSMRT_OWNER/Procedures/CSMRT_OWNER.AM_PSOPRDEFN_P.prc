DROP PROCEDURE CSMRT_OWNER.AM_PSOPRDEFN_P
/

--
-- AM_PSOPRDEFN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PSOPRDEFN_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PSOPRDEFN';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PSOPRDEFN@AMSOURCE S)
 where TABLE_NAME = 'PSOPRDEFN'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PSOPRDEFN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PSOPRDEFN';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PSOPRDEFN T 
using (select /*+ full(S) */
...
from SYSADM.PSOPRDEFN@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSOPRDEFN') ) S 
 on ( 
    T.OPRID = S.OPRID and 
    T.SRC_SYS_ID = 'CS90')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PSOPRDEFN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PSOPRDEFN';
update AMSTG_OWNER.PSOPRDEFN T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select 1 from
(select OPRID
   from AMSTG_OWNER.PSOPRDEFN T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSOPRDEFN') = 'Y'
  minus
 select OPRID
   from SYSADM.PSOPRDEFN@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSOPRDEFN') = 'Y' 
   ) S
