DROP PROCEDURE CSMRT_OWNER.PS_PERSON_CHECKLST_P
/

--
-- PS_PERSON_CHECKLST_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_PERSON_CHECKLST_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_PERSON_CHECKLST';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_PERSON_CHECKLST@SASOURCE S)
 where TABLE_NAME = 'PS_PERSON_CHECKLST'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_PERSON_CHECKLST';
...
insert /*+ append */  into CSSTG_OWNER.PS_T_PERSON_CHECKLST
select /*+ full(S) */
...
from SYSADM.PS_PERSON_CHECKLST@SASOURCE S
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_PERSON_CHECKLST';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_PERSON_CHECKLST';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_PERSON_CHECKLST T
using (select /*+ full(S) */
    nvl(trim(COMMON_ID),'-') COMMON_ID,
    nvl(SEQ_3C,0) SEQ_3C,
    nvl(trim(SA_ID_TYPE),'-') SA_ID_TYPE,
...
from CSSTG_OWNER.PS_T_PERSON_CHECKLST S
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_CHECKLST') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_PERSON_CHECKLST';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_PERSON_CHECKLST';
update CSSTG_OWNER.PS_PERSON_CHECKLST T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select COMMON_ID, SEQ_3C
   from CSSTG_OWNER.PS_PERSON_CHECKLST T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_CHECKLST') = 'Y'
  minus
 select COMMON_ID, SEQ_3C
   from SYSADM.PS_PERSON_CHECKLST@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_CHECKLST') = 'Y'
   ) S
