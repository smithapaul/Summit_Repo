DROP PROCEDURE CSMRT_OWNER.PS_CLASS_ATTRIBUTE_P
/

--
-- PS_CLASS_ATTRIBUTE_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_CLASS_ATTRIBUTE_P"
   AUTHID CURRENT_USER
IS
   /*
...
BEGIN
   ...
   UPDATE CSSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_CLASS_ATTRIBUTE@SASOURCE S)
    WHERE TABLE_NAME = 'PS_CLASS_ATTRIBUTE';
   ...
   strMessage01 := 'Merging data into CSSTG_OWNER.PS_CLASS_ATTRIBUTE';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into CSSTG_OWNER.PS_CLASS_ATTRIBUTE';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_CLASS_ATTRIBUTE T
using (select /*+ full(S) */
...
from SYSADM.PS_CLASS_ATTRIBUTE@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_ATTRIBUTE') ) S
   on (
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_CLASS_ATTRIBUTE';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on CSSTG_OWNER.PS_CLASS_ATTRIBUTE';

update CSSTG_OWNER.PS_CLASS_ATTRIBUTE T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION, CRSE_ATTR, CRSE_ATTR_VALUE
   from CSSTG_OWNER.PS_CLASS_ATTRIBUTE T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_ATTRIBUTE') = 'Y'
  minus
 select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION, CRSE_ATTR, CRSE_ATTR_VALUE
   from SYSADM.PS_CLASS_ATTRIBUTE@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_ATTRIBUTE') = 'Y' 
   ) S
