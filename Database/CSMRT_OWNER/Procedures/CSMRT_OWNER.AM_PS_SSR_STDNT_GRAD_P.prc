DROP PROCEDURE CSMRT_OWNER.AM_PS_SSR_STDNT_GRAD_P
/

--
-- AM_PS_SSR_STDNT_GRAD_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_SSR_STDNT_GRAD_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_SSR_STDNT_GRAD@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_SSR_STDNT_GRAD';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_SSR_STDNT_GRAD';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_SSR_STDNT_GRAD';
   
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_SSR_STDNT_GRAD T 
using (select /*+ full(S) */ 
...
from SYSADM.PS_SSR_STDNT_GRAD@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SSR_STDNT_GRAD') 
...
values ( 
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_SSR_STDNT_GRAD';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_SSR_STDNT_GRAD';

   UPDATE AMSTG_OWNER.PS_SSR_STDNT_GRAD T
      ...
      AND EXISTS
                 (SELECT 1
                    FROM (SELECT EMPLID, INSTITUTION, ACAD_CAREER, STDNT_CAR_NBR, ACAD_PROG, nvl(trim(EXP_GRAD_TERM),'-') EXP_GRAD_TERM, DEGREE
                            FROM AMSTG_OWNER.PS_SSR_STDNT_GRAD T2
                          ...
                          SELECT EMPLID, INSTITUTION, ACAD_CAREER, STDNT_CAR_NBR, ACAD_PROG, nvl(trim(EXP_GRAD_TERM),'-') EXP_GRAD_TERM, DEGREE
                            FROM SYSADM.PS_SSR_STDNT_GRAD@AMSOURCE
                           WHERE (SELECT DELETE_FLG
                                    FROM AMSTG_OWNER.UM_STAGE_JOBS
