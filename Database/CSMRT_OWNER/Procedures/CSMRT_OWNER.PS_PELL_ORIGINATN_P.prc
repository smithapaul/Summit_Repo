DROP PROCEDURE CSMRT_OWNER.PS_PELL_ORIGINATN_P
/

--
-- PS_PELL_ORIGINATN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_PELL_ORIGINATN_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_PELL_ORIGINATN';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_PELL_ORIGINATN@SASOURCE S)
 where TABLE_NAME = 'PS_PELL_ORIGINATN'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_PELL_ORIGINATN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_PELL_ORIGINATN';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_PELL_ORIGINATN T
using (select /*+ full(S) */
...
to_date(to_char(case when PELL_MRR_STAT_DT < '01-JAN-1800' then NULL else PELL_MRR_STAT_DT end,'MM/DD/YYYY HH24:MI:SS'),'MM/DD/YYYY HH24:MI:SS') PELL_MRR_STAT_DT
  from SYSADM.PS_PELL_ORIGINATN@SASOURCE S
 where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_ORIGINATN')
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_PELL_ORIGINATN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_PELL_ORIGINATN';
update CSSTG_OWNER.PS_PELL_ORIGINATN T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, PELL_ORIG_ID
   from CSSTG_OWNER.PS_PELL_ORIGINATN T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_ORIGINATN') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, PELL_ORIG_ID
   from SYSADM.PS_PELL_ORIGINATN@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_ORIGINATN') = 'Y'
   ) S
