DROP PROCEDURE CSMRT_OWNER.PS_CAMPUS_EVNT_ATT_P
/

--
-- PS_CAMPUS_EVNT_ATT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_CAMPUS_EVNT_ATT_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_CAMPUS_EVNT_ATT';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_CAMPUS_EVNT_ATT@SASOURCE S)
 where TABLE_NAME = 'PS_CAMPUS_EVNT_ATT'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_CAMPUS_EVNT_ATT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_CAMPUS_EVNT_ATT';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_CAMPUS_EVNT_ATT T
using (select /*+ full(S) */
...
from SYSADM.PS_CAMPUS_EVNT_ATT@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CAMPUS_EVNT_ATT') ) S
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_CAMPUS_EVNT_ATT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_CAMPUS_EVNT_ATT';
update CSSTG_OWNER.PS_CAMPUS_EVNT_ATT T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select CAMPUS_EVENT_NBR, CAMPUS_EVENT_ATND
   from CSSTG_OWNER.PS_CAMPUS_EVNT_ATT T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CAMPUS_EVNT_ATT') = 'Y'
  minus
 select CAMPUS_EVENT_NBR, CAMPUS_EVENT_ATND
   from SYSADM.PS_CAMPUS_EVNT_ATT@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CAMPUS_EVNT_ATT') = 'Y' 
   ) S
