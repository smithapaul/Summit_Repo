DROP PROCEDURE CSMRT_OWNER.AM_PS_CITIZENSHIP_P
/

--
-- AM_PS_CITIZENSHIP_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_CITIZENSHIP_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_CITIZENSHIP';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_CITIZENSHIP@AMSOURCE S)
 where TABLE_NAME = 'PS_CITIZENSHIP'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_CITIZENSHIP';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_CITIZENSHIP';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_CITIZENSHIP T
using (select /*+ full(S) */
...
from SYSADM.PS_CITIZENSHIP@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CITIZENSHIP') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_CITIZENSHIP';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_CITIZENSHIP';
update AMSTG_OWNER.PS_CITIZENSHIP T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select nvl(trim(EMPLID),'-') EMPLID, 
    nvl(trim(DEPENDENT_ID),'-') DEPENDENT_ID, 
    nvl(trim(COUNTRY),'-') COUNTRY
   from AMSTG_OWNER.PS_CITIZENSHIP T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CITIZENSHIP') = 'Y'
  minus
 select nvl(trim(EMPLID),'-') EMPLID, 
    nvl(trim(DEPENDENT_ID),'-') DEPENDENT_ID, 
    nvl(trim(COUNTRY),'-') COUNTRY
   from SYSADM.PS_CITIZENSHIP@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CITIZENSHIP') = 'Y' 
   ) S
