DROP PROCEDURE CSMRT_OWNER.PS_ACCOM_TYPE_TBL_P
/

--
-- PS_ACCOM_TYPE_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_ACCOM_TYPE_TBL_P"
   AUTHID CURRENT_USER
IS
   /*
...
BEGIN
   ...
   UPDATE CSSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_ACCOM_TYPE_TBL@SASOURCE S)
    WHERE TABLE_NAME = 'PS_ACCOM_TYPE_TBL';
   ...
   strMessage01 := 'Merging data into CSSTG_OWNER.PS_ACCOM_TYPE_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into CSSTG_OWNER.PS_ACCOM_TYPE_TBL';

   MERGE /*+ use_hash(S,T) */
        INTO  CSSTG_OWNER.PS_ACCOM_TYPE_TBL T
        USING (SELECT /*+ full(S) */
   ...
   strMessage01 := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_ACCOM_TYPE_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on CSSTG_OWNER.PS_ACCOM_TYPE_TBL';

   UPDATE CSSTG_OWNER.PS_ACCOM_TYPE_TBL T
      SET T.DATA_ORIGIN = 'D', T.LASTUPD_EW_DTTM = SYSDATE
    WHERE     T.DATA_ORIGIN <> 'D'
          AND EXISTS
                 (SELECT 1
                    FROM (SELECT ACCOMMODATION_TYPE, EFFDT
                            FROM CSSTG_OWNER.PS_ACCOM_TYPE_TBL T2
                          ...
                          SELECT ACCOMMODATION_TYPE, EFFDT
                            FROM SYSADM.PS_ACCOM_TYPE_TBL@SASOURCE
                           WHERE (SELECT DELETE_FLG
                                    FROM CSSTG_OWNER.UM_STAGE_JOBS
                                   WHERE TABLE_NAME = 'PS_ACCOM_TYPE_TBL') =
                                    'Y') S
