DROP PROCEDURE CSMRT_OWNER.PS_INSTR_TERM_DTL_P
/

--
-- PS_INSTR_TERM_DTL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_INSTR_TERM_DTL_P"
   AUTHID CURRENT_USER
IS
   /*
...
BEGIN
   ...
   UPDATE CSSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_INSTR_TERM_DTL@SASOURCE S)
    WHERE TABLE_NAME = 'PS_INSTR_TERM_DTL';
   ...
   strMessage01 := 'Merging data into CSSTG_OWNER.PS_INSTR_TERM_DTL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into CSSTG_OWNER.PS_INSTR_TERM_DTL';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_INSTR_TERM_DTL T
using (select /*+ full(S) */
...
from SYSADM.PS_INSTR_TERM_DTL@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_INSTR_TERM_DTL') ) S
   on (
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_INSTR_TERM_DTL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on CSSTG_OWNER.PS_INSTR_TERM_DTL';

update CSSTG_OWNER.PS_INSTR_TERM_DTL T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, STRM, INST_TRM_DET_SEQ
   from CSSTG_OWNER.PS_INSTR_TERM_DTL T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_INSTR_TERM_DTL') = 'Y'
  minus
 select EMPLID, INSTITUTION, STRM, INST_TRM_DET_SEQ
   from SYSADM.PS_INSTR_TERM_DTL@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_INSTR_TERM_DTL') = 'Y' 
   ) S
