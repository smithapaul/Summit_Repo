DROP PROCEDURE CSMRT_OWNER.PS_BUDGET_ITEM_TBL_P
/

--
-- PS_BUDGET_ITEM_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_BUDGET_ITEM_TBL_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_BUDGET_ITEM_TBL';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_BUDGET_ITEM_TBL@SASOURCE S)
 where TABLE_NAME = 'PS_BUDGET_ITEM_TBL'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_BUDGET_ITEM_TBL';
...
insert /*+ append */  into CSSTG_OWNER.PS_T_BUDGET_ITEM_TBL
select /*+ full(S) */
...
from SYSADM.PS_BUDGET_ITEM_TBL@SASOURCE S
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_BUDGET_ITEM_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_BUDGET_ITEM_TBL';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_BUDGET_ITEM_TBL T
using (select /*+ full(S) */
    nvl(trim(INSTITUTION),'-') INSTITUTION, 
    nvl(trim(AID_YEAR),'-') AID_YEAR, 
    nvl(trim(BGT_ITEM_CATEGORY),'-') BGT_ITEM_CATEGORY, 
    ...
    DESCRLONG DESCRLONG
from CSSTG_OWNER.PS_T_BUDGET_ITEM_TBL S 
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_BUDGET_ITEM_TBL') ) S
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_BUDGET_ITEM_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_BUDGET_ITEM_TBL';
update CSSTG_OWNER.PS_BUDGET_ITEM_TBL T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select INSTITUTION, AID_YEAR, BGT_ITEM_CATEGORY, BUDGET_ITEM_CD
   from CSSTG_OWNER.PS_BUDGET_ITEM_TBL T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_BUDGET_ITEM_TBL') = 'Y'
  minus
 select INSTITUTION, AID_YEAR, BGT_ITEM_CATEGORY, BUDGET_ITEM_CD
   from SYSADM.PS_BUDGET_ITEM_TBL@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_BUDGET_ITEM_TBL') = 'Y'
   ) S
