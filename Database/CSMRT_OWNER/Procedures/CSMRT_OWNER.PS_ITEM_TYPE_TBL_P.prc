DROP PROCEDURE CSMRT_OWNER.PS_ITEM_TYPE_TBL_P
/

--
-- PS_ITEM_TYPE_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_ITEM_TYPE_TBL_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_ITEM_TYPE_TBL';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ITEM_TYPE_TBL@SASOURCE S)
 where TABLE_NAME = 'PS_ITEM_TYPE_TBL'
;
...
from SYSADM.PS_ITEM_TYPE_TBL@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ITEM_TYPE_TBL') ) S
 on ( 
    T.SETID = S.SETID and 
...
(select SETID, ITEM_TYPE, EFFDT
  ...
  minus
 select SETID, ITEM_TYPE, EFFDT
   from SYSADM.PS_ITEM_TYPE_TBL@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ITEM_TYPE_TBL') = 'Y'
   ) S
