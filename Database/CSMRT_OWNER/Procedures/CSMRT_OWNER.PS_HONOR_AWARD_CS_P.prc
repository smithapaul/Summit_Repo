DROP PROCEDURE CSMRT_OWNER.PS_HONOR_AWARD_CS_P
/

--
-- PS_HONOR_AWARD_CS_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_HONOR_AWARD_CS_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_HONOR_AWARD_CS';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_HONOR_AWARD_CS@SASOURCE S)
 where TABLE_NAME = 'PS_HONOR_AWARD_CS'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_HONOR_AWARD_CS';
...
INSERT /*+ append */
      INTO  CSSTG_OWNER.PS_T_HONOR_AWARD_CS
   SELECT /*+ full(S) */
         EMPLID,
          'CS90' SRC_SYS_ID,
          DT_RECVD,
     ...
     FROM SYSADM.PS_HONOR_AWARD_CS@SASOURCE
;
strSqlCommand := 'commit';
commit;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_HONOR_AWARD_CS';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_HONOR_AWARD_CS';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_HONOR_AWARD_CS T
using (with Q1 as (
select /*+ full(S) */
...
from CSSTG_OWNER.PS_T_HONOR_AWARD_CS S
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_HONOR_AWARD_CS'))
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_HONOR_AWARD_CS';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_HONOR_AWARD_CS';
update CSSTG_OWNER.PS_HONOR_AWARD_CS T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID,  DT_RECVD, INSTITUTION, ACAD_CAREER, STRM, AWARD_CODE
   from CSSTG_OWNER.PS_HONOR_AWARD_CS T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_HONOR_AWARD_CS') = 'Y'
  minus
 select EMPLID,  DT_RECVD, INSTITUTION, nvl(trim(ACAD_CAREER),'-'), nvl(trim(STRM),'-'), AWARD_CODE
   from SYSADM.PS_HONOR_AWARD_CS@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_HONOR_AWARD_CS') = 'Y'
   ) S
