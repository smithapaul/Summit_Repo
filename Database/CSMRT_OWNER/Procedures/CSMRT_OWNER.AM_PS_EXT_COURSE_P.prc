DROP PROCEDURE CSMRT_OWNER.AM_PS_EXT_COURSE_P
/

--
-- AM_PS_EXT_COURSE_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_EXT_COURSE_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_EXT_COURSE';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_EXT_COURSE@AMSOURCE S)
 where TABLE_NAME = 'PS_EXT_COURSE'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_EXT_COURSE';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_EXT_COURSE';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_EXT_COURSE T 
using (select /*+ full(S) */
...
from SYSADM.PS_EXT_COURSE@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_COURSE')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_EXT_COURSE';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_EXT_COURSE';
update AMSTG_OWNER.PS_EXT_COURSE T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, EXT_ORG_ID, EXT_COURSE_NBR
   from AMSTG_OWNER.PS_EXT_COURSE T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_COURSE') = 'Y'
  minus
 select EMPLID, EXT_ORG_ID, EXT_COURSE_NBR
   from SYSADM.PS_EXT_COURSE@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EXT_COURSE') = 'Y' 
   ) S
