DROP PROCEDURE CSMRT_OWNER.AM_PS_ACAD_ORG_FS_OWN_P
/

--
-- AM_PS_ACAD_ORG_FS_OWN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_ACAD_ORG_FS_OWN_P IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_ACAD_ORG_FS_OWN@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_ACAD_ORG_FS_OWN';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_ACAD_ORG_FS_OWN';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_ACAD_ORG_FS_OWN';
   
   
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ACAD_ORG_FS_OWN T
using (select /*+ full(S) */
...
from SYSADM.PS_ACAD_ORG_FS_OWN@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_ORG_FS_OWN') ) S
   on (
...
sysdate
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_ORG_FS_OWN';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_ORG_FS_OWN';

update AMSTG_OWNER.PS_ACAD_ORG_FS_OWN T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select BUSINESS_UNIT, DEPTID, EFFDT, ACAD_ORG,PERCENT_OWNED
   from AMSTG_OWNER.PS_ACAD_ORG_FS_OWN T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_ORG_FS_OWN') = 'Y'
  minus
 select BUSINESS_UNIT, DEPTID, EFFDT, ACAD_ORG,PERCENT_OWNED
   from SYSADM.PS_ACAD_ORG_FS_OWN@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_ORG_FS_OWN') = 'Y' 
   ) S
