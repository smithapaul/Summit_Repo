DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_RESPONSE_P
/

--
-- AM_PS_STDNT_RESPONSE_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_RESPONSE_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STDNT_RESPONSE';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STDNT_RESPONSE@AMSOURCE S)
 where TABLE_NAME = 'PS_STDNT_RESPONSE'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_STDNT_RESPONSE';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_STDNT_RESPONSE';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_RESPONSE T 
using (select /*+ full(S) */
    ...
    DESCR DESCR
from SYSADM.PS_STDNT_RESPONSE@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_RESPONSE')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_RESPONSE';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_RESPONSE';
update AMSTG_OWNER.PS_STDNT_RESPONSE T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR, SEQNUM
   from AMSTG_OWNER.PS_STDNT_RESPONSE T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_RESPONSE') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR, SEQNUM
   from SYSADM.PS_STDNT_RESPONSE@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_RESPONSE') = 'Y' 
   ) S
