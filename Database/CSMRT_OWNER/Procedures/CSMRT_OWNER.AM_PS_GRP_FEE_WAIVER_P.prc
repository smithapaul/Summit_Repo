DROP PROCEDURE CSMRT_OWNER.AM_PS_GRP_FEE_WAIVER_P
/

--
-- AM_PS_GRP_FEE_WAIVER_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_GRP_FEE_WAIVER_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_GRP_FEE_WAIVER';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_GRP_FEE_WAIVER@AMSOURCE S)
 where TABLE_NAME = 'PS_GRP_FEE_WAIVER'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_GRP_FEE_WAIVER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_GRP_FEE_WAIVER';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_GRP_FEE_WAIVER T 
using (select /*+ full(S) */
...
from SYSADM.PS_GRP_FEE_WAIVER@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRP_FEE_WAIVER') ) S 
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_GRP_FEE_WAIVER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_GRP_FEE_WAIVER';
update AMSTG_OWNER.PS_GRP_FEE_WAIVER T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select nvl(trim(BUSINESS_UNIT),'-') BUSINESS_UNIT, 
        ...
        nvl(trim(FEE_CODE),'-') FEE_CODE, 
        nvl(trim(WAIVER_CODE),'-') WAIVER_CODE
   from AMSTG_OWNER.PS_GRP_FEE_WAIVER T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRP_FEE_WAIVER') = 'Y'
  minus
 select nvl(trim(BUSINESS_UNIT),'-') BUSINESS_UNIT, 
        nvl(trim(SEL_GROUP),'-') SEL_GROUP, 
        to_date(to_char(case when EFFDT < '01-JAN-1800' then NULL 
        ...
        nvl(trim(FEE_CODE),'-') FEE_CODE, 
        nvl(trim(WAIVER_CODE),'-') WAIVER_CODE
   from SYSADM.PS_GRP_FEE_WAIVER@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRP_FEE_WAIVER') = 'Y'
   ) S
