...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_ACCOUNT_SF';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ACCOUNT_SF@SASOURCE S)
 where TABLE_NAME = 'PS_ACCOUNT_SF'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_ACCOUNT_SF';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_ACCOUNT_SF';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_ACCOUNT_SF T 
    using (select /*+ full(S) */
  ...
  from SYSADM.PS_ACCOUNT_SF@SASOURCE S
 where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACCOUNT_SF')
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_ACCOUNT_SF';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_ACCOUNT_SF';
update CSSTG_OWNER.PS_ACCOUNT_SF T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select BUSINESS_UNIT, EMPLID, ACCOUNT_NBR, ACCOUNT_TERM
   from CSSTG_OWNER.PS_ACCOUNT_SF T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACCOUNT_SF') = 'Y'
  minus
 select BUSINESS_UNIT, EMPLID, ACCOUNT_NBR, ACCOUNT_TERM
   from SYSADM.PS_ACCOUNT_SF@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACCOUNT_SF') = 'Y'
   ) S
