DROP PROCEDURE CSMRT_OWNER.AM_PS_PELL_DISBMNT_P
/

--
-- AM_PS_PELL_DISBMNT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_PELL_DISBMNT_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_PELL_DISBMNT';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_PELL_DISBMNT@AMSOURCE S)
 where TABLE_NAME = 'PS_PELL_DISBMNT'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_PELL_DISBMNT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_PELL_DISBMNT';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_PELL_DISBMNT T
using (select /*+ full(S) */
...
nvl(trim(SFA_COD_ENR_SCHLCD),'-') SFA_COD_ENR_SCHLCD
  from SYSADM.PS_PELL_DISBMNT@AMSOURCE S
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_DISBMNT')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_PELL_DISBMNT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_PELL_DISBMNT';
update AMSTG_OWNER.PS_PELL_DISBMNT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, PELL_ORIG_ID, PELL_DISB_SEQ_NBR
   from AMSTG_OWNER.PS_PELL_DISBMNT T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_DISBMNT') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, PELL_ORIG_ID, PELL_DISB_SEQ_NBR
   from SYSADM.PS_PELL_DISBMNT@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_DISBMNT') = 'Y'
   ) S
