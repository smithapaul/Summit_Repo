DROP PROCEDURE CSMRT_OWNER.PS_SRVC_IND_CD_TBL_P
/

--
-- PS_SRVC_IND_CD_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_SRVC_IND_CD_TBL_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_SRVC_IND_CD_TBL';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_SRVC_IND_CD_TBL@SASOURCE S)
 where TABLE_NAME = 'PS_SRVC_IND_CD_TBL'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_SRVC_IND_CD_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_SRVC_IND_CD_TBL';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_SRVC_IND_CD_TBL T
using (select /*+ full(S) */
...
from SYSADM.PS_SRVC_IND_CD_TBL@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SRVC_IND_CD_TBL') ) S
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_SRVC_IND_CD_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_SRVC_IND_CD_TBL';
update CSSTG_OWNER.PS_SRVC_IND_CD_TBL T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select INSTITUTION, SRVC_IND_CD, EFFDT
   from CSSTG_OWNER.PS_SRVC_IND_CD_TBL T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SRVC_IND_CD_TBL') = 'Y'
  minus
 select INSTITUTION, SRVC_IND_CD, EFFDT
   from SYSADM.PS_SRVC_IND_CD_TBL@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SRVC_IND_CD_TBL') = 'Y'
   ) S
