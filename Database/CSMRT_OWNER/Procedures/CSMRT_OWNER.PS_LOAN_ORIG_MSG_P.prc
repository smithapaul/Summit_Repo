DROP PROCEDURE CSMRT_OWNER.PS_LOAN_ORIG_MSG_P
/

--
-- PS_LOAN_ORIG_MSG_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_LOAN_ORIG_MSG_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_LOAN_ORIG_MSG';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_LOAN_ORIG_MSG@SASOURCE S)
 where TABLE_NAME = 'PS_LOAN_ORIG_MSG'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_LOAN_ORIG_MSG';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_LOAN_ORIG_MSG';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_LOAN_ORIG_MSG T
using (select /*+ full(S) */
...
nvl(trim(LN_ACTION_MSG),'-') LN_ACTION_MSG
  from SYSADM.PS_LOAN_ORIG_MSG@SASOURCE S
 where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOAN_ORIG_MSG')
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_LOAN_ORIG_MSG';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_LOAN_ORIG_MSG';
update CSSTG_OWNER.PS_LOAN_ORIG_MSG T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, ACAD_CAREER, LOAN_TYPE, LN_APPL_SEQ, ITEM_TYPE, LN_ORIG_ACTN_SEQ, LNORIG_MSG_SEQ
   from CSSTG_OWNER.PS_LOAN_ORIG_MSG T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOAN_ORIG_MSG') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, ACAD_CAREER, LOAN_TYPE, LN_APPL_SEQ, ITEM_TYPE, LN_ORIG_ACTN_SEQ, LNORIG_MSG_SEQ
   from SYSADM.PS_LOAN_ORIG_MSG@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOAN_ORIG_MSG') = 'Y'
   ) S
