DROP PROCEDURE CSMRT_OWNER.UM_AF_AMH_STUDENT_ENROLL_S2_P
/

--
-- UM_AF_AMH_STUDENT_ENROLL_S2_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.UM_AF_AMH_STUDENT_ENROLL_S2_P AUTHID CURRENT_USER IS
   ...
   select COLUMN_001 INSTITUTION_CD,
          ...
          --to_date(COLUMN_038,'YYYY-MM-DD') BIRTHDATE,   -- Jan 2022 
          to_date(NVL(trim(column_038), to_date('20010101','YYYYMMDD')),'YYYY-MM-DD') BIRTHDATE,  -- Jan 2022 
          trim(COLUMN_039) SEX, 
          trim(COLUMN_040) ETHNIC_GRP_CD, 
          trim(COLUMN_041) CITIZENSHIP_STATUS, 
          trim(COLUMN_042) CITIZENSHIP_STATUS_LD, 
          trim(COLUMN_043) UM_CITIZENSHIP, 
          trim(COLUMN_044) UM_CITIZENSHIP_LD, 
          trim(COLUMN_045) MILITARY_STATUS, 
          trim(COLUMN_046) MILITARY_STATUS_LD, 
          trim(COLUMN_047) UM_PRIMARY_STATE, 
          trim(COLUMN_048) UM_PRIMARY_POSTAL, 
          SYSDATE CREATED_EW_DTTM
     from COMMON_OWNER.UPLOAD_S1_VW        -- Reads from appropriate database with DB link
--     from COMMON_OWNER.UPLOAD_S1    -- Temp!!! 
   ...
   select INSTITUTION_CD, INSTITUTION_LD, TERM_CD, TERM_LD, ACAD_YR, AID_YEAR, PERSON_ID,
          ACAD_CAR_CD, ACAD_CAR_LD, CE_ONLY_FLG, ACAD_ORG_CD, ACAD_ORG_LD, ACAD_LEVEL_BOT,
          ...
          BIRTHDATE, SEX, ETHNIC_GRP_CD, 
          CITIZENSHIP_STATUS, CITIZENSHIP_STATUS_LD, UM_CITIZENSHIP, UM_CITIZENSHIP_LD, MILITARY_STATUS, MILITARY_STATUS_LD, 
          UM_PRIMARY_STATE, UM_PRIMARY_POSTAL, 
          CREATED_EW_DTTM
     from Q1
   ;
