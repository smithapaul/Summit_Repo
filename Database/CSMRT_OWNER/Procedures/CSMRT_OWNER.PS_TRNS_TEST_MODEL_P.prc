DROP PROCEDURE CSMRT_OWNER.PS_TRNS_TEST_MODEL_P
/

--
-- PS_TRNS_TEST_MODEL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_TRNS_TEST_MODEL_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_TRNS_TEST_MODEL';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_TRNS_TEST_MODEL@SASOURCE S)
 where TABLE_NAME = 'PS_TRNS_TEST_MODEL'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_TRNS_TEST_MODEL';
...
insert /*+ append */  into CSSTG_OWNER.PS_T_TRNS_TEST_MODEL
select /*+ full(S) */
...
from SYSADM.PS_TRNS_TEST_MODEL@SASOURCE S 
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_TRNS_TEST_MODEL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_TRNS_TEST_MODEL';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_TRNS_TEST_MODEL T
using (select /*+ full(S) */
    nvl(trim(EMPLID),'-') EMPLID, 
    nvl(trim(ACAD_CAREER),'-') ACAD_CAREER, 
    nvl(trim(INSTITUTION),'-') INSTITUTION, 
    ...
    nvl(trim(INCLUDE_IN_GPA),'-') INCLUDE_IN_GPA, 
    COMMENTS
from CSSTG_OWNER.PS_T_TRNS_TEST_MODEL S 
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_TEST_MODEL') ) S
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_TRNS_TEST_MODEL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_TRNS_TEST_MODEL';
update CSSTG_OWNER.PS_TRNS_TEST_MODEL T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, INSTITUTION, MODEL_NBR
   from CSSTG_OWNER.PS_TRNS_TEST_MODEL T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_TEST_MODEL') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, INSTITUTION, MODEL_NBR
   from SYSADM.PS_TRNS_TEST_MODEL@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_TEST_MODEL') = 'Y'
   ) S
