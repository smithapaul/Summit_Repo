DROP PROCEDURE CSMRT_OWNER.AM_PS_HEALTH_TEST_P
/

--
-- AM_PS_HEALTH_TEST_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_HEALTH_TEST_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_HEALTH_TEST';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_HEALTH_TEST@AMSOURCE S)
 where TABLE_NAME = 'PS_HEALTH_TEST'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_HEALTH_TEST';
...
insert /*+ append */  into AMSTG_OWNER.PS_T_HEALTH_TEST
select /*+ full(S) */
       EMPLID, 
       HEALTH_TEST, 
       HEALTH_SEQ, 
  ...
  from SYSADM.PS_HEALTH_TEST@AMSOURCE S
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_HEALTH_TEST';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_HEALTH_TEST';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_HEALTH_TEST T
    using (select /*+ full(S) */
...
--    nvl(trim(TEST_RESULT),'-') TEST_RESULT, 
    TEST_RESULT,
    to_char(substr(trim(COMMENTS), 1, 4000)) COMMENTS
from AMSTG_OWNER.PS_T_HEALTH_TEST S
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_HEALTH_TEST') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_HEALTH_TEST';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_HEALTH_TEST';
update AMSTG_OWNER.PS_HEALTH_TEST T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, HEALTH_TEST, HEALTH_SEQ
   from AMSTG_OWNER.PS_HEALTH_TEST T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_HEALTH_TEST') = 'Y'
  minus
 select EMPLID, HEALTH_TEST, HEALTH_SEQ
   from SYSADM.PS_HEALTH_TEST@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_HEALTH_TEST') = 'Y'
   ) S
