DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_AWRD_ACTV_P
/

--
-- AM_PS_STDNT_AWRD_ACTV_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_AWRD_ACTV_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STDNT_AWRD_ACTV';
...
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_AWRD_ACTV T
using (select /*+ full(S) */
...
update AMSTG_OWNER.PS_STDNT_AWRD_ACTV T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, ITEM_TYPE, ACAD_CAREER, ACTION_DTTM
  ...
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_AWRD_ACTV') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, ITEM_TYPE, ACAD_CAREER, ACTION_DTTM
