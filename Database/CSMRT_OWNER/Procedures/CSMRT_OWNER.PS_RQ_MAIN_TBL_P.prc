DROP PROCEDURE CSMRT_OWNER.PS_RQ_MAIN_TBL_P
/

--
-- PS_RQ_MAIN_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_RQ_MAIN_TBL_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_RQ_MAIN_TBL';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_RQ_MAIN_TBL@SASOURCE S)
 where TABLE_NAME = 'PS_RQ_MAIN_TBL'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_RQ_MAIN_TBL';
...
insert /*+ append */  into CSSTG_OWNER.PS_T_RQ_MAIN_TBL
select REQUIREMENT, 
       EFFDT, 
       'CS90' 
       SRC_SYS_ID, 
  ...
  from SYSADM.PS_RQ_MAIN_TBL@SASOURCE
  ;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_RQ_MAIN_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_RQ_MAIN_TBL';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_RQ_MAIN_TBL T
using (select /*+ full(S) */
...
from CSSTG_OWNER.PS_T_RQ_MAIN_TBL S
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_RQ_MAIN_TBL')
) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_RQ_MAIN_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_RQ_MAIN_TBL';
update CSSTG_OWNER.PS_RQ_MAIN_TBL T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select REQUIREMENT, EFFDT
   from CSSTG_OWNER.PS_RQ_MAIN_TBL T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_RQ_MAIN_TBL') = 'Y'
  minus
 select REQUIREMENT, EFFDT
   from SYSADM.PS_RQ_MAIN_TBL@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_RQ_MAIN_TBL') = 'Y'
   ) S
