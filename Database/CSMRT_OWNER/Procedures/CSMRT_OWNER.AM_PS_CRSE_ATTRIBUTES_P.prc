DROP PROCEDURE CSMRT_OWNER.AM_PS_CRSE_ATTRIBUTES_P
/

--
-- AM_PS_CRSE_ATTRIBUTES_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_CRSE_ATTRIBUTES_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_CRSE_ATTRIBUTES@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_CRSE_ATTRIBUTES';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_CRSE_ATTRIBUTES';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_CRSE_ATTRIBUTES';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_CRSE_ATTRIBUTES T
using (select /*+ full(S) */
...
from SYSADM.PS_CRSE_ATTRIBUTES@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CRSE_ATTRIBUTES') ) S
   on (
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_CRSE_ATTRIBUTES';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_CRSE_ATTRIBUTES';

update AMSTG_OWNER.PS_CRSE_ATTRIBUTES T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select CRSE_ID, EFFDT, CRSE_ATTR, CRSE_ATTR_VALUE
   from AMSTG_OWNER.PS_CRSE_ATTRIBUTES T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CRSE_ATTRIBUTES') = 'Y'
  minus
 select CRSE_ID, EFFDT, CRSE_ATTR, CRSE_ATTR_VALUE
   from SYSADM.PS_CRSE_ATTRIBUTES@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CRSE_ATTRIBUTES') = 'Y' 
   ) S
