DROP PROCEDURE CSMRT_OWNER.AM_PS_ISIR_00_2_EC_P
/

--
-- AM_PS_ISIR_00_2_EC_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ISIR_00_2_EC_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ISIR_00_2_EC';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ISIR_00_2_EC@AMSOURCE S)
 where TABLE_NAME = 'PS_ISIR_00_2_EC'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ISIR_00_2_EC';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ISIR_00_2_EC';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ISIR_00_2_EC T
using (select /*+ full(S) */
...
from SYSADM.PS_ISIR_00_2_EC@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_00_2_EC') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ISIR_00_2_EC';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ISIR_00_2_EC';
update AMSTG_OWNER.PS_ISIR_00_2_EC T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select ECTRANSID, ECQUEUEINSTANCE, ECTRANSINOUTSW, ISIR_SEQ_NO
   from AMSTG_OWNER.PS_ISIR_00_2_EC T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_00_2_EC') = 'Y'
  minus
 select ECTRANSID, ECQUEUEINSTANCE, ECTRANSINOUTSW, ISIR_SEQ_NO
   from SYSADM.PS_ISIR_00_2_EC@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_00_2_EC') = 'Y'
   ) S
