DROP PROCEDURE CSMRT_OWNER.AM_PS_PERSON_SA_P
/

--
-- AM_PS_PERSON_SA_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_PERSON_SA_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_PERSON_SA@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_PERSON_SA';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_PERSON_SA';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

strSqlCommand := 'merge into AMSTG_OWNER.PS_PERSON_SA';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_PERSON_SA T
using (select /*+ full(S) */
...
from SYSADM.PS_PERSON_SA@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_SA') 
...
;
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_PERSON_SA';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_PERSON_SA';
update AMSTG_OWNER.PS_PERSON_SA T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select 1 from
(select EMPLID
   from AMSTG_OWNER.PS_PERSON_SA T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_SA') = 'Y'
  minus
 select EMPLID
   from SYSADM.PS_PERSON_SA@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERSON_SA') = 'Y'
   ) S
