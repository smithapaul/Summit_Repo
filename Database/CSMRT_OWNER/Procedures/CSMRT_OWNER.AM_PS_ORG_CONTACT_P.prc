DROP PROCEDURE CSMRT_OWNER.AM_PS_ORG_CONTACT_P
/

--
-- AM_PS_ORG_CONTACT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ORG_CONTACT_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ORG_CONTACT';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ORG_CONTACT@AMSOURCE S)
 where TABLE_NAME = 'PS_ORG_CONTACT'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ORG_CONTACT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ORG_CONTACT';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ORG_CONTACT T
using (select /*+ full(S) */
...
from SYSADM.PS_ORG_CONTACT@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ORG_CONTACT') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ORG_CONTACT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ORG_CONTACT';
update AMSTG_OWNER.PS_ORG_CONTACT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EXT_ORG_ID, ORG_CONTACT, EFFDT
   from AMSTG_OWNER.PS_ORG_CONTACT T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ORG_CONTACT') = 'Y'
  minus
 select EXT_ORG_ID, ORG_CONTACT, EFFDT
   from SYSADM.PS_ORG_CONTACT@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ORG_CONTACT') = 'Y'
   ) S
