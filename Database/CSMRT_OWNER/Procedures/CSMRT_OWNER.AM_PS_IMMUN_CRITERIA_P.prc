DROP PROCEDURE CSMRT_OWNER.AM_PS_IMMUN_CRITERIA_P
/

--
-- AM_PS_IMMUN_CRITERIA_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_IMMUN_CRITERIA_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_IMMUN_CRITERIA';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_IMMUN_CRITERIA@AMSOURCE S)
 where TABLE_NAME = 'PS_IMMUN_CRITERIA'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_IMMUN_CRITERIA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_IMMUN_CRITERIA';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_IMMUN_CRITERIA T 
using (select /*+ full(S) */
...
from SYSADM.PS_IMMUN_CRITERIA@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_IMMUN_CRITERIA') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_IMMUN_CRITERIA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_IMMUN_CRITERIA';
update AMSTG_OWNER.PS_IMMUN_CRITERIA T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, IMMUNIZATION, IMMUN_SEQ, CRITERIA_SEQ
   from AMSTG_OWNER.PS_IMMUN_CRITERIA T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_IMMUN_CRITERIA') = 'Y'
  minus
 select EMPLID, IMMUNIZATION, IMMUN_SEQ, CRITERIA_SEQ
   from SYSADM.PS_IMMUN_CRITERIA@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_IMMUN_CRITERIA') = 'Y'
            and EMPLID between '00000000' and '99999999'
            and length(EMPLID) = 8  
   ) S
