DROP PROCEDURE CSMRT_OWNER.UM_S_SF_ITEM_LINE_P
/

--
-- UM_S_SF_ITEM_LINE_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."UM_S_SF_ITEM_LINE_P" (
    i_FY           IN VARCHAR2,
    i_Replace_FY   IN VARCHAR2 DEFAULT 'N')
    AUTHID CURRENT_USER
IS
    ------------------------------------------------------------------------
    -- The purpose of this procedure is to create snapshot table based on fact table UM_S_SF_ITEM_LINE using fiscal year parameters
    ...
    strProcessName         VARCHAR2 (100) := 'UM_S_SF_ITEM_LINE_P';
...
AND PRT.TABLE_NAME = 'UM_S_SF_ITEM_LINE')
                )                   PARTITION_COUNT
        FROM  DUAL
        ),
        (
...
ROW_NUMBER() OVER (ORDER BY to_number(SUBSTR(PRT.PARTITION_NAME,4)) ASC) row_id
  FROM ALL_TAB_PARTITIONS PRT
WHERE     PRT.TABLE_OWNER = 'CSMRT_OWNER'
AND PRT.TABLE_NAME = 'UM_S_SF_ITEM_LINE' ) where row_id =1) OLD_PARTITION_YEAR 
 FROM DUAL
 )
 ;
  DBMS_OUTPUT.PUT_LINE (
...
strSqlCommand := 'Validate Partitioncounter and drop Older Partition';             -----validate if table has more than 3 yrs of partitions and drop the oldest partition
   IF strPartitionCounterflg = 'Y' and strPartitionExists='N'
    THEN
     strSqlDynamic :=
               'ALTER TABLE CSMRT_OWNER.UM_S_SF_ITEM_LINE DROP PARTITION '
            || stroldPartitionYear
            || ' UPDATE INDEXES';
    ...
    THEN
        strSqlDynamic :=
               'ALTER TABLE CSMRT_OWNER.UM_S_SF_ITEM_LINE TRUNCATE PARTITION '
            || i_FY
            || ' UPDATE INDEXES';
        strSqlCommand := 'SMT_UTILITY.EXECUTE_IMMEDIATE: ' || strSqlDynamic;
        COMMON_OWNER.SMT_UTILITY.EXECUTE_IMMEDIATE (
    ...
    THEN
        V_REF_CUR_QUERY :=
               'ALTER TABLE CSMRT_OWNER.UM_S_SF_ITEM_LINE ADD PARTITION '
            || i_FY
            || ' values'
            || '('
            || strPartitionName
    ...
    strMessage01 :=
        'Disabling Indexes for table CSMRT_OWNER.UM_S_SF_ITEM_LINE';
    COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);
    COMMON_OWNER.SMT_INDEX.ALL_UNUSABLE ('CSMRT_OWNER', 'UM_S_SF_ITEM_LINE');
    DBMS_OUTPUT.PUT_LINE (strMessage01);

    strSqlDynamic :=
        'alter table CSMRT_OWNER.UM_S_SF_ITEM_LINE disable constraint PK_UM_S_SF_ITEM_LINE';
    strSqlCommand := 'SMT_UTILITY.EXECUTE_IMMEDIATE: ' || strSqlDynamic;
    COMMON_OWNER.SMT_UTILITY.EXECUTE_IMMEDIATE (
        i_SqlStatement   => strSqlDynamic,
        i_MaxTries       => 10,
    ...
    strMessage01 := 'Inserting into CSMRT_OWNER.UM_S_SF_ITEM_LINE';
    DBMS_OUTPUT.PUT_LINE (strMessage01);

    INSERT /*+ append parallel(8)*/
           INTO CSMRT_OWNER.UM_S_SF_ITEM_LINE
    SELECT strPartitionName AS  UM_FISCAL_YEAR,F1.*, sysdate as UM_SNAPSHOT_DT from CSMRT_OWNER.UM_F_SF_ITEM_LINE F1 
    ;
    ...
    strMessage01 :=
           '# of UM_S_SF_ITEM_LINE rows inserted: '
        || TO_CHAR (intRowCount, '999,999,999,999');
    COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

    strSqlCommand := 'SMT_PROCESS_LOG.PROCESS_DETAIL';
    COMMON_OWNER.SMT_PROCESS_LOG.PROCESS_DETAIL (
        i_TargetTableName   => 'UM_S_SF_ITEM_LINE',
        i_Action            => 'INSERT',
        i_RowCount          => intRowCount);


    strMessage01 :=
        'Enabling Indexes for table CSMRT_OWNER.UM_S_SF_ITEM_LINE';
    COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

    strSqlDynamic :=
        'alter table CSMRT_OWNER.UM_S_SF_ITEM_LINE enable constraint PK_UM_S_SF_ITEM_LINE';
    strSqlCommand := 'SMT_UTILITY.EXECUTE_IMMEDIATE: ' || strSqlDynamic;
    COMMON_OWNER.SMT_UTILITY.EXECUTE_IMMEDIATE (
        i_SqlStatement   => strSqlDynamic,
        i_MaxTries       => 10,
    ...
    strMessage01 :=
        'Rebuilding Indexes for table CSMRT_OWNER.UM_S_SF_ITEM_LINE';
    COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);
    COMMON_OWNER.SMT_INDEX.ALL_REBUILD ('CSMRT_OWNER', 'UM_S_SF_ITEM_LINE');
