DROP PROCEDURE CSMRT_OWNER.AM_PS_ADM_APPL_RCTER_P
/

--
-- AM_PS_ADM_APPL_RCTER_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ADM_APPL_RCTER_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ADM_APPL_RCTER';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ADM_APPL_RCTER@AMSOURCE S)
 where TABLE_NAME = 'PS_ADM_APPL_RCTER'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ADM_APPL_RCTER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ADM_APPL_RCTER';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ADM_APPL_RCTER T 
using (select /*+ full(S) */
...
from SYSADM.PS_ADM_APPL_RCTER@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_RCTER') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ADM_APPL_RCTER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ADM_APPL_RCTER';

update AMSTG_OWNER.PS_ADM_APPL_RCTER T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR, INSTITUTION, RECRUITMENT_CAT, RECRUITER_ID
   from AMSTG_OWNER.PS_ADM_APPL_RCTER T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_RCTER') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR, INSTITUTION, RECRUITMENT_CAT, RECRUITER_ID
   from SYSADM.PS_ADM_APPL_RCTER@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_RCTER') = 'Y' 
   ) S
