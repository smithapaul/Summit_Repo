DROP PROCEDURE CSMRT_OWNER.AM_PS_CLASS_INSTR_P
/

--
-- AM_PS_CLASS_INSTR_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_CLASS_INSTR_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_CLASS_INSTR@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_CLASS_INSTR';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_CLASS_INSTR';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_CLASS_INSTR';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_CLASS_INSTR T
using (select /*+ full(S) */
...
from SYSADM.PS_CLASS_INSTR@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_INSTR') ) S
   on (
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_CLASS_INSTR';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_CLASS_INSTR';

update AMSTG_OWNER.PS_CLASS_INSTR T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION, CLASS_MTG_NBR, INSTR_ASSIGN_SEQ
   from AMSTG_OWNER.PS_CLASS_INSTR T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_INSTR') = 'Y'
  minus
 select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, CLASS_SECTION, CLASS_MTG_NBR, INSTR_ASSIGN_SEQ
   from SYSADM.PS_CLASS_INSTR@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_INSTR') = 'Y' 
   ) S
