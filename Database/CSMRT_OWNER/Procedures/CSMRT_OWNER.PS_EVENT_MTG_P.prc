...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_EVENT_MTG';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_EVENT_MTG@SASOURCE S)
 where TABLE_NAME = 'PS_EVENT_MTG'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_EVENT_MTG';
...
INSERT /*+ append */
      INTO  CSSTG_OWNER.PS_T_EVENT_MTG
select /*+ full(S) */
...
from SYSADM.PS_EVENT_MTG@SASOURCE S 
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_EVENT_MTG';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_EVENT_MTG';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_EVENT_MTG T 
using (select /*+ full(S) */
    nvl(trim(CAMPUS_EVENT_NBR),'-') CAMPUS_EVENT_NBR, 
    nvl(EVENT_MTG_NBR,0) EVENT_MTG_NBR,
    'CS90' SRC_SYS_ID,    
    ...
    to_char(substr(trim(COMMENTS), 1, 4000)) COMMENTS,
    1234 BATCH_SID
from CSSTG_OWNER.PS_T_EVENT_MTG 
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EVENT_MTG') 
 ) S 
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_EVENT_MTG';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_EVENT_MTG';
update CSSTG_OWNER.PS_EVENT_MTG T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select CAMPUS_EVENT_NBR, EVENT_MTG_NBR
   from CSSTG_OWNER.PS_EVENT_MTG T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EVENT_MTG') = 'Y'
  minus
 select CAMPUS_EVENT_NBR, EVENT_MTG_NBR
   from SYSADM.PS_EVENT_MTG@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_EVENT_MTG') = 'Y'
   ) S
