DROP PROCEDURE CSMRT_OWNER.AM_PS_INSTR_ADVISOR_P
/

--
-- AM_PS_INSTR_ADVISOR_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_INSTR_ADVISOR_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_INSTR_ADVISOR@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_INSTR_ADVISOR';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_INSTR_ADVISOR';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_INSTR_ADVISOR';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_INSTR_ADVISOR T
using (select /*+ full(S) */
...
from SYSADM.PS_INSTR_ADVISOR@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_INSTR_ADVISOR') 
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_INSTR_ADVISOR';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_INSTR_ADVISOR';

update AMSTG_OWNER.PS_INSTR_ADVISOR T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, EFFDT
   from AMSTG_OWNER.PS_INSTR_ADVISOR T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_INSTR_ADVISOR') = 'Y'
  minus
 select EMPLID, INSTITUTION, EFFDT
   from SYSADM.PS_INSTR_ADVISOR@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_INSTR_ADVISOR') = 'Y' 
   ) S
