DROP PROCEDURE CSMRT_OWNER.PS_RESIDENCY_OFF_P
/

--
-- PS_RESIDENCY_OFF_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_RESIDENCY_OFF_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_RESIDENCY_OFF';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_RESIDENCY_OFF@SASOURCE S)
 where TABLE_NAME = 'PS_RESIDENCY_OFF'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_RESIDENCY_OFF';
...
insert /*+ append */  into CSSTG_OWNER.PS_T_RESIDENCY_OFF
select /*+ full(S) */
    ...
    from SYSADM.PS_RESIDENCY_OFF@SASOURCE S
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_RESIDENCY_OFF';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_RESIDENCY_OFF';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_RESIDENCY_OFF T
using (select /*+ full(S) */
    nvl(trim(EMPLID),'-') EMPLID, 
    nvl(trim(ACAD_CAREER),'-') ACAD_CAREER, 
    nvl(trim(INSTITUTION),'-') INSTITUTION, 
    ...
    nvl(trim(POSTAL),'-') POSTAL, 
    to_char(substr(trim(COMMENTS), 1, 4000)) COMMENTS
    from CSSTG_OWNER.PS_T_RESIDENCY_OFF S
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_RESIDENCY_OFF') ) S
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_RESIDENCY_OFF';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_RESIDENCY_OFF';
update CSSTG_OWNER.PS_RESIDENCY_OFF T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, INSTITUTION, EFFECTIVE_TERM
   from CSSTG_OWNER.PS_RESIDENCY_OFF T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_RESIDENCY_OFF') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, INSTITUTION, nvl(trim(EFFECTIVE_TERM),'-') EFFECTIVE_TERM
   from SYSADM.PS_RESIDENCY_OFF@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_RESIDENCY_OFF') = 'Y'
   ) S
