DROP PROCEDURE CSMRT_OWNER.AM_PS_CLASS_ASSOC_P
/

--
-- AM_PS_CLASS_ASSOC_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_CLASS_ASSOC_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_CLASS_ASSOC@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_CLASS_ASSOC';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_CLASS_ASSOC';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into AMSTG_OWNER.PS_CLASS_ASSOC';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_CLASS_ASSOC T
using (select /*+ full(S) */
...
from SYSADM.PS_CLASS_ASSOC@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_ASSOC') ) S
   on (
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_CLASS_ASSOC';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_CLASS_ASSOC';

update AMSTG_OWNER.PS_CLASS_ASSOC T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, ASSOCIATED_CLASS
   from AMSTG_OWNER.PS_CLASS_ASSOC T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_ASSOC') = 'Y'
  minus
 select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, ASSOCIATED_CLASS
   from SYSADM.PS_CLASS_ASSOC@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_ASSOC') = 'Y' 
   ) S
