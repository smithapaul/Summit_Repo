DROP PROCEDURE CSMRT_OWNER.AM_PSTREENODE_P
/

--
-- AM_PSTREENODE_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PSTREENODE_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PSTREENODE';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PSTREENODE@AMSOURCE S)
 where TABLE_NAME = 'PSTREENODE'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PSTREENODE';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PSTREENODE';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PSTREENODE T
using (select /*+ full(S) */
...
from SYSADM.PSTREENODE@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSTREENODE') ) S
 on ( 
    T.SETID = S.SETID and 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PSTREENODE';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PSTREENODE';
update AMSTG_OWNER.PSTREENODE T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select SETID, SETCNTRLVALUE, TREE_NAME, EFFDT, TREE_NODE_NUM, TREE_NODE, TREE_BRANCH
   from AMSTG_OWNER.PSTREENODE T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSTREENODE') = 'Y'
  minus
 select nvl(trim(SETID),'-') SETID, 
    ...
    nvl(trim(TREE_NODE),'-') TREE_NODE, 
    nvl(trim(TREE_BRANCH),'-') TREE_BRANCH
   from SYSADM.PSTREENODE@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSTREENODE') = 'Y'
   ) S
