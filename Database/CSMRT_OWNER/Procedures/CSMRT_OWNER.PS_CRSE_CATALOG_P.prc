DROP PROCEDURE CSMRT_OWNER.PS_CRSE_CATALOG_P
/

--
-- PS_CRSE_CATALOG_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_CRSE_CATALOG_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_CRSE_CATALOG';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_CRSE_CATALOG@SASOURCE S)
 where TABLE_NAME = 'PS_CRSE_CATALOG'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_CRSE_CATALOG';
...
INSERT /*+ append */
      INTO  CSSTG_OWNER.PS_T_CRSE_CATALOG
   SELECT /*+ full(S) */
         CRSE_ID,
          EFFDT,
          'CS90' SRC_SYS_ID,
     ...
     FROM SYSADM.PS_CRSE_CATALOG@SASOURCE;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_CRSE_CATALOG';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_CRSE_CATALOG';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_CRSE_CATALOG T
using (select /*+ full(S) */
...
DESCRLONG DESCRLONG
from CSSTG_OWNER.PS_T_CRSE_CATALOG S
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CRSE_CATALOG') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_CRSE_CATALOG';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_CRSE_CATALOG';
update CSSTG_OWNER.PS_CRSE_CATALOG T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select CRSE_ID, EFFDT
   from CSSTG_OWNER.PS_CRSE_CATALOG T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CRSE_CATALOG') = 'Y'
  minus
 select CRSE_ID, EFFDT
   from SYSADM.PS_CRSE_CATALOG@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CRSE_CATALOG') = 'Y'
   ) S
