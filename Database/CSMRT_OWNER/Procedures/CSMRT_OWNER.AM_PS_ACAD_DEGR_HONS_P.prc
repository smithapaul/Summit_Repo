DROP PROCEDURE CSMRT_OWNER.AM_PS_ACAD_DEGR_HONS_P
/

--
-- AM_PS_ACAD_DEGR_HONS_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ACAD_DEGR_HONS_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ACAD_DEGR_HONS';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ACAD_DEGR_HONS@AMSOURCE S)
 where TABLE_NAME = 'PS_ACAD_DEGR_HONS'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ACAD_DEGR_HONS';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ACAD_DEGR_HONS';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ACAD_DEGR_HONS T
using (select /*+ full(S) */
...
from SYSADM.PS_ACAD_DEGR_HONS@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_DEGR_HONS') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_DEGR_HONS';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ACAD_DEGR_HONS';
update AMSTG_OWNER.PS_ACAD_DEGR_HONS T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, STDNT_DEGR, HONORS_NBR
   from AMSTG_OWNER.PS_ACAD_DEGR_HONS T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_DEGR_HONS') = 'Y'
  minus
 select EMPLID, STDNT_DEGR,HONORS_NBR
   from SYSADM.PS_ACAD_DEGR_HONS@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ACAD_DEGR_HONS') = 'Y' 
