DROP PROCEDURE CSMRT_OWNER.AM_PS_PELL_ORIG_DTL_P
/

--
-- AM_PS_PELL_ORIG_DTL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_PELL_ORIG_DTL_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_PELL_ORIG_DTL';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_PELL_ORIG_DTL@AMSOURCE S)
 where TABLE_NAME = 'PS_PELL_ORIG_DTL'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_PELL_ORIG_DTL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_PELL_ORIG_DTL';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_PELL_ORIG_DTL T
using (select /*+ full(S) */
...
to_date(to_char(case when SFA_ATB_COMP_DT < '01-JAN-1800' then NULL else SFA_ATB_COMP_DT end,'MM/DD/YYYY HH24:MI:SS'),'MM/DD/YYYY HH24:MI:SS') SFA_ATB_COMP_DT
  from SYSADM.PS_PELL_ORIG_DTL@AMSOURCE S
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_ORIG_DTL')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_PELL_ORIG_DTL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_PELL_ORIG_DTL';
update AMSTG_OWNER.PS_PELL_ORIG_DTL T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, PELL_ORIG_ID, PELL_ORIG_SEQ_NBR
   from AMSTG_OWNER.PS_PELL_ORIG_DTL T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_ORIG_DTL') = 'Y'
  minus
 select EMPLID, INSTITUTION, AID_YEAR, PELL_ORIG_ID, PELL_ORIG_SEQ_NBR
   from SYSADM.PS_PELL_ORIG_DTL@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PELL_ORIG_DTL') = 'Y'
   ) S
