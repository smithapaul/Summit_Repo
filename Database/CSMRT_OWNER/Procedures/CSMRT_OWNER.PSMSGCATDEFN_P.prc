DROP PROCEDURE CSMRT_OWNER.PSMSGCATDEFN_P
/

--
-- PSMSGCATDEFN_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PSMSGCATDEFN_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PSMSGCATDEFN';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PSMSGCATDEFN@SASOURCE S)
 where TABLE_NAME = 'PSMSGCATDEFN'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_PSMSGCATDEFN';
...
INSERT /*+ append */
  INTO CSSTG_OWNER.PS_T_PSMSGCATDEFN
SELECT /*+ full(S) */
       MESSAGE_SET_NBR, 
       MESSAGE_NBR, 
       MESSAGE_TEXT, 
...
from SYSADM.PSMSGCATDEFN@SASOURCE S;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PSMSGCATDEFN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PSMSGCATDEFN';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PSMSGCATDEFN T
using (select /*+ full(S) */
	nvl(MESSAGE_SET_NBR,0) MESSAGE_SET_NBR, 
       ...
       DESCRLONG
from CSSTG_OWNER.PS_T_PSMSGCATDEFN S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSMSGCATDEFN') ) S
 on ( 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PSMSGCATDEFN';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PSMSGCATDEFN';
update CSSTG_OWNER.PSMSGCATDEFN T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select MESSAGE_SET_NBR, MESSAGE_NBR
   from CSSTG_OWNER.PSMSGCATDEFN T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSMSGCATDEFN') = 'Y'
  minus
 select MESSAGE_SET_NBR, MESSAGE_NBR
   from SYSADM.PSMSGCATDEFN@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PSMSGCATDEFN') = 'Y'
   ) S
