DROP PROCEDURE CSMRT_OWNER.PS_ISIR_STUDENT_P
/

--
-- PS_ISIR_STUDENT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_ISIR_STUDENT_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_ISIR_STUDENT';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ISIR_STUDENT@SASOURCE S)
 where TABLE_NAME = 'PS_ISIR_STUDENT'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_ISIR_STUDENT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_ISIR_STUDENT';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_ISIR_STUDENT T
using (select /*+ full(S) */
...
from SYSADM.PS_ISIR_STUDENT@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_STUDENT') ) S
   on (
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_ISIR_STUDENT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_ISIR_STUDENT';
update CSSTG_OWNER.PS_ISIR_STUDENT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, EFFDT, EFFSEQ, TABLE_ID
   from CSSTG_OWNER.PS_ISIR_STUDENT T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_STUDENT') = 'Y'
  minus
 select nvl(trim(EMPLID),'-'), nvl(trim(INSTITUTION),'-'), nvl(trim(AID_YEAR),'-'), EFFDT, EFFSEQ, nvl(trim(TABLE_ID),'-')
   from SYSADM.PS_ISIR_STUDENT@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_STUDENT') = 'Y'
   ) S
