DROP PROCEDURE CSMRT_OWNER.PS_PERS_DATA_EFFDT_P
/

--
-- PS_PERS_DATA_EFFDT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_PERS_DATA_EFFDT_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_PERS_DATA_EFFDT';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_PERS_DATA_EFFDT@SASOURCE S)
 where TABLE_NAME = 'PS_PERS_DATA_EFFDT'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_PERS_DATA_EFFDT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_PERS_DATA_EFFDT';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_PERS_DATA_EFFDT T
using (select /*+ full(S) */
...
from SYSADM.PS_PERS_DATA_EFFDT@SASOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERS_DATA_EFFDT') 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_PERS_DATA_EFFDT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_PERS_DATA_EFFDT';
update CSSTG_OWNER.PS_PERS_DATA_EFFDT T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select 1 from
(select EMPLID, EFFDT
   from CSSTG_OWNER.PS_PERS_DATA_EFFDT T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERS_DATA_EFFDT') = 'Y'
  minus
 select EMPLID, EFFDT
   from SYSADM.PS_PERS_DATA_EFFDT@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERS_DATA_EFFDT') = 'Y'
    and EMPLID between '00000000' and '99999999'
