DROP PROCEDURE CSMRT_OWNER.PS_CLASS_COMPONENT_P
/

--
-- PS_CLASS_COMPONENT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_CLASS_COMPONENT_P" AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_CLASS_COMPONENT';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_CLASS_COMPONENT@SASOURCE S)
 where TABLE_NAME = 'PS_CLASS_COMPONENT'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_CLASS_COMPONENT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_CLASS_COMPONENT';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_CLASS_COMPONENT T 
using (select /*+ full(S) */ 
...
from SYSADM.PS_CLASS_COMPONENT@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_COMPONENT') ) S 
 on (
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_CLASS_COMPONENT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_CLASS_COMPONENT';
update CSSTG_OWNER.PS_CLASS_COMPONENT T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, ASSOCIATED_CLASS, SSR_COMPONENT
   from CSSTG_OWNER.PS_CLASS_COMPONENT T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_COMPONENT') = 'Y'
  minus
 select CRSE_ID, CRSE_OFFER_NBR, STRM, SESSION_CODE, ASSOCIATED_CLASS, SSR_COMPONENT
   from SYSADM.PS_CLASS_COMPONENT@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_CLASS_COMPONENT') = 'Y' 
   ) S
