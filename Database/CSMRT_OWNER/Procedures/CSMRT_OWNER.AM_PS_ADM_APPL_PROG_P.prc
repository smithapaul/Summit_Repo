DROP PROCEDURE CSMRT_OWNER.AM_PS_ADM_APPL_PROG_P
/

--
-- AM_PS_ADM_APPL_PROG_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ADM_APPL_PROG_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ADM_APPL_PROG';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ADM_APPL_PROG@AMSOURCE S)
 where TABLE_NAME = 'PS_ADM_APPL_PROG'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ADM_APPL_PROG';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ADM_APPL_PROG';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ADM_APPL_PROG T
using (select /*+ full(S) */
  ...
  from SYSADM.PS_ADM_APPL_PROG@AMSOURCE S 
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_PROG')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ADM_APPL_PROG';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ADM_APPL_PROG';
update AMSTG_OWNER.PS_ADM_APPL_PROG T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR, APPL_PROG_NBR, EFFDT, EFFSEQ
   from AMSTG_OWNER.PS_ADM_APPL_PROG T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_PROG') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR, APPL_PROG_NBR, EFFDT, EFFSEQ
   from SYSADM.PS_ADM_APPL_PROG@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_PROG') = 'Y'
   ) S
