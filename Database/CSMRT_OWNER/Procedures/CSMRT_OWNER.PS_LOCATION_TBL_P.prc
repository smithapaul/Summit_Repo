DROP PROCEDURE CSMRT_OWNER.PS_LOCATION_TBL_P
/

--
-- PS_LOCATION_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_LOCATION_TBL_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_LOCATION_TBL';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_LOCATION_TBL@SASOURCE S)
 where TABLE_NAME = 'PS_LOCATION_TBL'
;
...
strSqlDynamic   := 'truncate table CSSTG_OWNER.PS_T_LOCATION_TBL';
...
insert /*+ append */  into CSSTG_OWNER.PS_T_LOCATION_TBL
select /*+ full(S) */
...
from SYSADM.PS_LOCATION_TBL@SASOURCE S
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_LOCATION_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_LOCATION_TBL';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_LOCATION_TBL T 
using (select /*+ full(S) */
    nvl(trim(SETID),'-') SETID, 
    nvl(trim(LOCATION),'-') LOCATION, 
    EFFDT, 
    ...
    '-' SOC_SEC_WRK_CTR, 
    to_char(substr(trim(COMMENTS_2000), 1, 4000)) COMMENTS_2000
from CSSTG_OWNER.PS_T_LOCATION_TBL S 
where SRC_SCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOCATION_TBL') ) S 
 on ( 
    T.SETID = S.SETID and 
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_LOCATION_TBL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_LOCATION_TBL';
update CSSTG_OWNER.PS_LOCATION_TBL T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select SETID, LOCATION, EFFDT
   from CSSTG_OWNER.PS_LOCATION_TBL T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOCATION_TBL') = 'Y'
  minus
 select SETID, LOCATION, EFFDT
   from SYSADM.PS_LOCATION_TBL@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_LOCATION_TBL') = 'Y'
   ) S
