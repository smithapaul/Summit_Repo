CREATE OR REPLACE PROCEDURE             "UM_F_STDNT_ADVR_P" AUTHID CURRENT_USER IS
...
insert /*+ append */ into CSMRT_OWNER.UM_F_STDNT_ADVR 
  with X as ( 
...
select /*+ parallel(8) inline */
       ...
       S.TERM_BEGIN_DT,  
       S.TERM_END_DT  
  from UM_F_STDNT_ACAD_STRUCT S     -- PK --> TERM_SID, PERSON_SID, STDNT_CAR_NUM, ACAD_PLAN_SID, ACAD_SPLAN_SID, SRC_SYS_ID 
  left outer join Q2
    on Q2.TERM_SID = S.TERM_SID
   and Q2.PERSON_SID = S.PERSON_SID
   and Q2.ACAD_PROG_SID = S.ACAD_PROG_SID
   and Q2.ACAD_PLAN_SID = S.ACAD_PLAN_SID
   and Q2.SRC_SYS_ID = S.SRC_SYS_ID
