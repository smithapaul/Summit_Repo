DROP PROCEDURE CSMRT_OWNER.PS_GRADE_TBL_P
/

--
-- PS_GRADE_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_GRADE_TBL_P"
   AUTHID CURRENT_USER
IS
   /*
...
BEGIN
   ...
   UPDATE CSSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_GRADE_TBL@SASOURCE S)
    WHERE TABLE_NAME = 'PS_GRADE_TBL';
   ...
   strMessage01 := 'Merging data into CSSTG_OWNER.PS_GRADE_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into CSSTG_OWNER.PS_GRADE_TBL';

merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_GRADE_TBL T
using (select /*+ full(S) */
...
from SYSADM.PS_GRADE_TBL@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRADE_TBL') ) S
   on (
T.SETID = S.SETID and
...
COMMIT;
   ...
   strMessage01 := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_GRADE_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on CSSTG_OWNER.PS_GRADE_TBL';


update CSSTG_OWNER.PS_GRADE_TBL T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select SETID, GRADING_SCHEME, EFFDT, GRADING_BASIS, CRSE_GRADE_INPUT
   from CSSTG_OWNER.PS_GRADE_TBL T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRADE_TBL') = 'Y'
  minus
 select SETID, GRADING_SCHEME, EFFDT, GRADING_BASIS, CRSE_GRADE_INPUT
   from SYSADM.PS_GRADE_TBL@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_GRADE_TBL') = 'Y' 
   ) S
