DROP PROCEDURE CSMRT_OWNER.AM_PS_ISIR_PARENT_P
/

--
-- AM_PS_ISIR_PARENT_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_ISIR_PARENT_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ISIR_PARENT';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ISIR_PARENT@AMSOURCE S)
 where TABLE_NAME = 'PS_ISIR_PARENT'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ISIR_PARENT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ISIR_PARENT';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ISIR_PARENT T
using (select /*+ full(S) */
...
from SYSADM.PS_ISIR_PARENT@AMSOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_PARENT') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ISIR_PARENT';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ISIR_PARENT';
update AMSTG_OWNER.PS_ISIR_PARENT T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, AID_YEAR, EFFDT, EFFSEQ, TABLE_ID
   from AMSTG_OWNER.PS_ISIR_PARENT T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_PARENT') = 'Y'
  minus
 select nvl(trim(EMPLID),'-'), nvl(trim(INSTITUTION),'-'), nvl(trim(AID_YEAR),'-'), EFFDT, EFFSEQ, nvl(trim(TABLE_ID),'-')
   from SYSADM.PS_ISIR_PARENT@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ISIR_PARENT') = 'Y'
   ) S
