DROP PROCEDURE CSMRT_OWNER.AM_PS_PERS_DATA_USA_P
/

--
-- AM_PS_PERS_DATA_USA_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_PERS_DATA_USA_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_PERS_DATA_USA';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_PERS_DATA_USA@AMSOURCE S)
 where TABLE_NAME = 'PS_PERS_DATA_USA'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_PERS_DATA_USA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_PERS_DATA_USA';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_PERS_DATA_USA T
using (select /*+ full(S) */
...
from SYSADM.PS_PERS_DATA_USA@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERS_DATA_USA') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_PERS_DATA_USA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_PERS_DATA_USA';
update AMSTG_OWNER.PS_PERS_DATA_USA T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select 1 from
(select EMPLID, EFFDT
   from AMSTG_OWNER.PS_PERS_DATA_USA T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERS_DATA_USA') = 'Y'
  minus
 select EMPLID, EFFDT
   from SYSADM.PS_PERS_DATA_USA@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_PERS_DATA_USA') = 'Y'
   ) S
