DROP PROCEDURE CSMRT_OWNER.AM_PS_ADM_APPL_RCR_CA_P
/

--
-- AM_PS_ADM_APPL_RCR_CA_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_ADM_APPL_RCR_CA_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_ADM_APPL_RCR_CA';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_ADM_APPL_RCR_CA@AMSOURCE S)
 where TABLE_NAME = 'PS_ADM_APPL_RCR_CA'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_ADM_APPL_RCR_CA';
...
insert /*+ append */  into AMSTG_OWNER.PS_T_ADM_APPL_RCR_CA
select /*+ full(S) */
...
from SYSADM.PS_ADM_APPL_RCR_CA@AMSOURCE S 
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_ADM_APPL_RCR_CA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_ADM_APPL_RCR_CA';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_ADM_APPL_RCR_CA T
using (select /*+ full(S) */
    nvl(trim(EMPLID),'-') EMPLID, 
    nvl(trim(ACAD_CAREER),'-') ACAD_CAREER, 
    nvl(STDNT_CAR_NBR,0) STDNT_CAR_NBR, 
    ...
    nvl(trim(RECRUIT_SUB_CAT),'-') RECRUIT_SUB_CAT, 
    DESCRLONG
from AMSTG_OWNER.PS_T_ADM_APPL_RCR_CA S 
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_RCR_CA')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_ADM_APPL_RCR_CA';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_ADM_APPL_RCR_CA';
update AMSTG_OWNER.PS_ADM_APPL_RCR_CA T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR, INSTITUTION, RECRUITMENT_CAT
   from AMSTG_OWNER.PS_ADM_APPL_RCR_CA T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_RCR_CA') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, STDNT_CAR_NBR, ADM_APPL_NBR, INSTITUTION, RECRUITMENT_CAT
   from SYSADM.PS_ADM_APPL_RCR_CA@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_ADM_APPL_RCR_CA') = 'Y'
   ) S
