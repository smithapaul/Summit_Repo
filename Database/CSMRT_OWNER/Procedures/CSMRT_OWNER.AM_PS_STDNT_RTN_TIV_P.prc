DROP PROCEDURE CSMRT_OWNER.AM_PS_STDNT_RTN_TIV_P
/

--
-- AM_PS_STDNT_RTN_TIV_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_STDNT_RTN_TIV_P" IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_STDNT_RTN_TIV';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_STDNT_RTN_TIV@AMSOURCE S)
 where TABLE_NAME = 'PS_STDNT_RTN_TIV'
;
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_STDNT_RTN_TIV';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_STDNT_RTN_TIV';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_STDNT_RTN_TIV T
using (select /*+ full(S) */
...
nvl(STU_RTN_WK_11,0) STU_RTN_WK_11
  from SYSADM.PS_STDNT_RTN_TIV@AMSOURCE S
 where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_RTN_TIV')
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_RTN_TIV';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_STDNT_RTN_TIV';
update AMSTG_OWNER.PS_STDNT_RTN_TIV T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select INSTITUTION, AID_YEAR, EMPLID, STRM, RTRN_TIV_PGM_TYPE
   from AMSTG_OWNER.PS_STDNT_RTN_TIV T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_RTN_TIV') = 'Y'
  minus
 select INSTITUTION, AID_YEAR, EMPLID, STRM, RTRN_TIV_PGM_TYPE
   from SYSADM.PS_STDNT_RTN_TIV@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_STDNT_RTN_TIV') = 'Y'
   ) S
