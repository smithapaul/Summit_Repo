DROP PROCEDURE CSMRT_OWNER.PS_INSTRUCTOR_TERM_P
/

--
-- PS_INSTRUCTOR_TERM_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_INSTRUCTOR_TERM_P"
   AUTHID CURRENT_USER
IS
   /*
...
BEGIN
   ...
   UPDATE CSSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_INSTRUCTOR_TERM@SASOURCE S)
    WHERE TABLE_NAME = 'PS_INSTRUCTOR_TERM';
   ...
   strMessage01 := 'Merging data into CSSTG_OWNER.PS_INSTRUCTOR_TERM';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into CSSTG_OWNER.PS_INSTRUCTOR_TERM';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_INSTRUCTOR_TERM T
using (select /*+ full(S) */
...
from SYSADM.PS_INSTRUCTOR_TERM@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_INSTRUCTOR_TERM') ) S
   on (
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_INSTRUCTOR_TERM';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on CSSTG_OWNER.PS_INSTRUCTOR_TERM';

update CSSTG_OWNER.PS_INSTRUCTOR_TERM T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, INSTITUTION, STRM
   from CSSTG_OWNER.PS_INSTRUCTOR_TERM T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_INSTRUCTOR_TERM') = 'Y'
  minus
 select EMPLID, INSTITUTION, STRM
   from SYSADM.PS_INSTRUCTOR_TERM@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_INSTRUCTOR_TERM') = 'Y' 
   ) S
