DROP PROCEDURE CSMRT_OWNER.PS_BI_BILL_HEADER_P
/

--
-- PS_BI_BILL_HEADER_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.PS_BI_BILL_HEADER_P AUTHID CURRENT_USER IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'PS_BI_BILL_HEADER';
...
update CSSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Merging',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_BI_BILL_HEADER@SASOURCE S)
 where TABLE_NAME = 'PS_BI_BILL_HEADER'
;
...
strMessage01    := 'Merging data into CSSTG_OWNER.PS_BI_BILL_HEADER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into CSSTG_OWNER.PS_BI_BILL_HEADER';
merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_BI_BILL_HEADER T 
    using (select /*+ full(S) */
  ...
  from SYSADM.PS_BI_BILL_HEADER@SASOURCE S
 where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_BI_BILL_HEADER')
...
strMessage01    := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_BI_BILL_HEADER';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on CSSTG_OWNER.PS_BI_BILL_HEADER';
update CSSTG_OWNER.PS_BI_BILL_HEADER T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select BUSINESS_UNIT, INVOICE_ID
   from CSSTG_OWNER.PS_BI_BILL_HEADER T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_BI_BILL_HEADER') = 'Y'
  minus
 select BUSINESS_UNIT, INVOICE_ID
   from SYSADM.PS_BI_BILL_HEADER@SASOURCE S2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_BI_BILL_HEADER') = 'Y'
   ) S
