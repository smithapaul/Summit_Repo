DROP PROCEDURE CSMRT_OWNER.AM_PS_TRNS_TEST_DTL_P
/

--
-- AM_PS_TRNS_TEST_DTL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER.AM_PS_TRNS_TEST_DTL_P IS
...
------------------------------------------------------------------------
        ...
        strProcessName                  Varchar2(100)   := 'AM_PS_TRNS_TEST_DTL';
...
update AMSTG_OWNER.UM_STAGE_JOBS
   set TABLE_STATUS = 'Truncating',
       NEW_MAX_SCN = (select /*+ full(S) */ max(ORA_ROWSCN) from SYSADM.PS_TRNS_TEST_DTL@AMSOURCE S)
 where TABLE_NAME = 'PS_TRNS_TEST_DTL'
;
...
strSqlDynamic   := 'truncate table AMSTG_OWNER.PS_T_TRNS_TEST_DTL';
...
insert /*+ append */  into AMSTG_OWNER.PS_T_TRNS_TEST_DTL
select /*+ full(S) */
...
from SYSADM.PS_TRNS_TEST_DTL@AMSOURCE S 
...
strMessage01    := 'Merging data into AMSTG_OWNER.PS_TRNS_TEST_DTL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'merge into AMSTG_OWNER.PS_TRNS_TEST_DTL';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_TRNS_TEST_DTL T
using (select /*+ full(S) */
    nvl(trim(EMPLID),'-') EMPLID, 
    nvl(trim(ACAD_CAREER),'-') ACAD_CAREER, 
    nvl(trim(INSTITUTION),'-') INSTITUTION, 
    ...
    nvl(trim(SSR_FAWI_INCL),'-') SSR_FAWI_INCL, 
    COMMENTS COMMENTS
from AMSTG_OWNER.PS_T_TRNS_TEST_DTL S 
where SRC_SCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_TEST_DTL') 
...
strMessage01    := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_TRNS_TEST_DTL';
COMMON_OWNER.SMT_LOG.PUT_MESSAGE(i_Message => strMessage01);

strSqlCommand   := 'update DATA_ORIGIN on AMSTG_OWNER.PS_TRNS_TEST_DTL';
update AMSTG_OWNER.PS_TRNS_TEST_DTL T
   set T.DATA_ORIGIN = 'D',
          T.LASTUPD_EW_DTTM = SYSDATE
...
(select EMPLID, ACAD_CAREER, INSTITUTION, MODEL_NBR, ARTICULATION_TERM, TRNSFR_EQVLNCY_GRP, TRNSFR_EQVLNCY_SEQ
   from AMSTG_OWNER.PS_TRNS_TEST_DTL T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_TEST_DTL') = 'Y'
  minus
 select EMPLID, ACAD_CAREER, INSTITUTION, MODEL_NBR, ARTICULATION_TERM, TRNSFR_EQVLNCY_GRP, TRNSFR_EQVLNCY_SEQ
   from SYSADM.PS_TRNS_TEST_DTL@AMSOURCE S2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_TRNS_TEST_DTL') = 'Y'
   ) S
