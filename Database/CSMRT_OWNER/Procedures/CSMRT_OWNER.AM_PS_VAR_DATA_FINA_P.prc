DROP PROCEDURE CSMRT_OWNER.AM_PS_VAR_DATA_FINA_P
/

--
-- AM_PS_VAR_DATA_FINA_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."AM_PS_VAR_DATA_FINA_P" IS
...
BEGIN
   ...
   UPDATE AMSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_VAR_DATA_FINA@AMSOURCE S)
    WHERE TABLE_NAME = 'PS_VAR_DATA_FINA';
   ...
   strMessage01 := 'Merging data into AMSTG_OWNER.PS_VAR_DATA_FINA';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

strSqlCommand := 'merge into AMSTG_OWNER.PS_VAR_DATA_FINA';
merge /*+ use_hash(S,T) */ into AMSTG_OWNER.PS_VAR_DATA_FINA T
using (select /*+ full(S) */
    nvl(trim(COMMON_ID),'-') COMMON_ID, 
    nvl(VAR_DATA_SEQ,0) VAR_DATA_SEQ, 
    nvl(trim(AID_YEAR),'-') AID_YEAR
from SYSADM.PS_VAR_DATA_FINA@AMSOURCE S 
where ORA_ROWSCN > (select OLD_MAX_SCN from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_VAR_DATA_FINA') 
...
;
   ...
   strMessage01 := 'Updating DATA_ORIGIN on AMSTG_OWNER.PS_VAR_DATA_FINA';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

strSqlCommand := 'update DATA_ORIGIN on AMSTG_OWNER.PS_VAR_DATA_FINA';
update AMSTG_OWNER.PS_VAR_DATA_FINA T
   set T.DATA_ORIGIN = 'D',
       T.LASTUPD_EW_DTTM = SYSDATE
...
(select COMMON_ID, VAR_DATA_SEQ
   from AMSTG_OWNER.PS_VAR_DATA_FINA T2
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_VAR_DATA_FINA') = 'Y'
  minus
 select COMMON_ID, VAR_DATA_SEQ
   from SYSADM.PS_VAR_DATA_FINA@AMSOURCE
  where (select DELETE_FLG from AMSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_VAR_DATA_FINA') = 'Y'
   ) S
