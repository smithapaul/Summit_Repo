DROP PROCEDURE CSMRT_OWNER.PS_SCTN_CMBND_TBL_P
/

--
-- PS_SCTN_CMBND_TBL_P  (Procedure) 
--
CREATE OR REPLACE PROCEDURE CSMRT_OWNER."PS_SCTN_CMBND_TBL_P"
   AUTHID CURRENT_USER
IS
   /*
...
BEGIN
   ...
   UPDATE CSSTG_OWNER.UM_STAGE_JOBS
      SET TABLE_STATUS = 'Merging',
          NEW_MAX_SCN =
             (SELECT /*+ full(S) */
                    MAX (ORA_ROWSCN)
                FROM SYSADM.PS_SCTN_CMBND_TBL@SASOURCE S)
    WHERE TABLE_NAME = 'PS_SCTN_CMBND_TBL';
   ...
   strMessage01 := 'Merging data into CSSTG_OWNER.PS_SCTN_CMBND_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'merge into CSSTG_OWNER.PS_SCTN_CMBND_TBL';

merge /*+ use_hash(S,T) */ into CSSTG_OWNER.PS_SCTN_CMBND_TBL T
using (select /*+ full(S) */
...
from SYSADM.PS_SCTN_CMBND_TBL@SASOURCE S
where ORA_ROWSCN > (select OLD_MAX_SCN from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SCTN_CMBND_TBL') ) S
   on (
...
1234);
   ...
   strMessage01 := 'Updating DATA_ORIGIN on CSSTG_OWNER.PS_SCTN_CMBND_TBL';
   COMMON_OWNER.SMT_LOG.PUT_MESSAGE (i_Message => strMessage01);

   strSqlCommand := 'update DATA_ORIGIN on CSSTG_OWNER.PS_SCTN_CMBND_TBL';


update CSSTG_OWNER.PS_SCTN_CMBND_TBL T
        set T.DATA_ORIGIN = 'D',
               T.LASTUPD_EW_DTTM = SYSDATE
...
(select INSTITUTION, STRM, SESSION_CODE, SCTN_COMBINED_ID
   from CSSTG_OWNER.PS_SCTN_CMBND_TBL T2
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SCTN_CMBND_TBL') = 'Y'
  minus
 select INSTITUTION, STRM, SESSION_CODE, SCTN_COMBINED_ID
   from SYSADM.PS_SCTN_CMBND_TBL@SASOURCE
  where (select DELETE_FLG from CSSTG_OWNER.UM_STAGE_JOBS where TABLE_NAME = 'PS_SCTN_CMBND_TBL') = 'Y' 
   ) S
